...
import { assertUserScope, validateUserDocumentPath } from "../../lib/security/firestore-validation";
...
describe("firestore-validation", () => {
  describe("assertUserScope", () => {
    ...
    it("throws when userId is empty", () => {
      assert.throws(() => assertUserScope({ userId: "" }), /requires a user id/);
    });

    it("throws when userId contains invalid characters", () => {
      assert.throws(() => assertUserScope({ userId: "user@evil.com" }), /invalid user id format/);
    });

    it("throws when userId contains path traversal", () => {
      assert.throws(() => assertUserScope({ userId: "../admin" }), /invalid user id format/);
    });

    it("throws when userId contains spaces", () => {
      assert.throws(() => assertUserScope({ userId: "user name" }), /invalid user id format/);
    });

    it("throws when userId contains slashes", () => {
      assert.throws(() => assertUserScope({ userId: "user/other" }), /invalid user id format/);
    });
