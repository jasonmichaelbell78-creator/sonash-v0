...
describe("check-backlog-health: CONFIG defaults", () => {
  function getConfig(): Record<string, number> {
    return {
      S1_MAX_DAYS: 7,
      S2_MAX_DAYS: 14,
      MAX_ITEMS: 25,
  ...
  it("S1 threshold is 7 days", () => {
    assert.strictEqual(getConfig()["S1_MAX_DAYS"], 7);
  });

  it("S2 threshold is 14 days", () => {
    assert.strictEqual(getConfig()["S2_MAX_DAYS"], 14);
  });

  it("MAX_ITEMS threshold is 25", () => {
    assert.strictEqual(getConfig()["MAX_ITEMS"], 25);
  });
});
