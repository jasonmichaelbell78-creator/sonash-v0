...
describe("4.2d: Issue Templates", () => {
  ...
  describe("config.yml", () => {
    ...
    it("references Security Policy", () => {
      assert.ok(content.includes("SECURITY.md"), "Should reference SECURITY.md");
    });
  });
});
...
describe("4.3a: Copilot Instructions", () => {
  ...
  it("has updated test count (~3776)", () => {
    assert.ok(content.includes("3776"), "Should reference updated test count of ~3776");
  });

  it("does not have stale 77/91 test count", () => {
    assert.ok(!content.includes("77/91"), "Should not contain old 77/91 test count");
  });

  it("does not have stale ESLint ~115 warnings reference", () => {
    assert.ok(!content.includes("~115 warnings"), "Should not contain old ~115 warnings");
  });
  ...
  it("includes format check in validation steps", () => {
    assert.ok(content.includes("format:check"), "Should include Prettier format check");
  });

  it("includes pattern check in validation steps", () => {
    assert.ok(content.includes("patterns:check"), "Should include pattern compliance check");
  });
});
...
describe("4.5: Dependabot", () => {
  ...
  it("has grouping for actions", () => {
    assert.ok(content.includes("actions-minor-patch"), "Should have actions grouping");
  });
...
describe("4.6: Codecov Integration", () => {
  describe("CI workflow", () => {
    ...
    it("has codecov action step", () => {
      assert.ok(content.includes("codecov/codecov-action"), "CI should include codecov action");
    });
    ...
    it("has version comment", () => {
      ...
      const lines = content.split("\n");
      const codecovLine = lines.findIndex((l) => l.includes("codecov/codecov-action@"));
...
describe("4.7: OpenSSF Scorecard", () => {
  ...
  it("has version comment", () => {
    const lines = content.split("\n");
    const scLine = lines.findIndex((l) => l.includes("ossf/scorecard-action@"));
    assert.ok(scLine >= 0);
...
describe("4.10: GitHub Guide", () => {
  ...
  it("has all 9 sections", () => {
    ...
    for (const section of requiredSections) {
      assert.ok(content.includes(section), `Should have section: ${section}`);
    }
  });
  ...
  it("references key files", () => {
    ...
    for (const file of keyFiles) {
      assert.ok(content.includes(file), `Should reference: ${file}`);
    }
  });
});
    ...
    describe(wf, () => {
      ...
      it("all uses: entries are SHA-pinned or local", () => {
        ...
        for (const line of usesLines) {
          ...
          assert.ok(shaMatch, `Action should be SHA-pinned: ${trimmed}`);
        }
      });
      ...
      it("SHA-pinned actions have version comments", () => {
        ...
        const lines = content.split("\n");
        const usesLines = lines.filter((l) => l.trim().startsWith("uses:") && l.includes("@"));
  ...
  for (const doc of docs) {
    describe(doc, () => {
      ...
      it("has Purpose section", () => {
        ...
        assert.ok(content.includes("Purpose"), `${doc} should have Purpose section`);
      });
