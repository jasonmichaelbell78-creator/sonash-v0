...
import { COLLECTIONS, buildUserPath } from "../lib/db/collections";
...
describe("collections utilities", () => {
  describe("COLLECTIONS constants", () => {
    ...
    it("generates user subcollection paths", () => {
      ...
      assert.strictEqual(COLLECTIONS.userJournal(userId), "users/test-user-123/journal");
      assert.strictEqual(COLLECTIONS.userDailyLogs(userId), "users/test-user-123/daily_logs");
      assert.strictEqual(COLLECTIONS.userInventory(userId), "users/test-user-123/inventory");
      assert.strictEqual(COLLECTIONS.userWorksheets(userId), "users/test-user-123/worksheets");
    });
  });
