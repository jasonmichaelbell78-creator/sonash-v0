...
**Date:** 2026-03-12 **Questions Asked:** 26 **Decisions Captured:** 26

## Cross-Cutting Directives

These user directives apply to ALL workstreams and override any individual
...
3. **Warnings must demand action (block, not inform)** — warnings without action
   are useless
4. **Guardrails AND fix problems** — applies to EVERYTHING, not just individual
   workstreams
...
8. **All files are AI-maintained** — MD is for human consumption, JSONL is for
   AI consumption
9. **Final reconciliation required** — parse ROADMAP, planning files,
   MASTER_DEBT for completed items
...
| 2   | ESLint sonash plugin API migration ordering | Migrate sonash plugin to forward-compatible APIs FIRST (v10-ready while still on v9), then warning triage. API migration may surface new warnings.                                                | Sonash plugin uses deprecated v9 APIs that will be removed in v10. Migrating now ensures forward compatibility and a clean triage baseline. Actual v10 upgrade deferred until v10 is released. |
