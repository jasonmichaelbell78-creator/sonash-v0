...
must_haves: truths: - "Test fixtures exist for all 3 completeness tiers across
reviews, retros, and deferred-items" - "Every fixture passes its corresponding
Zod schema validation" - "Contract tests verify that data produced by one
pipeline stage is consumable by the next" - "Contract tests pass with full,
partial, and stub fixture data without throwing" artifacts: - path:
...
key_links: - from: "tests/scripts/ecosystem-v2/contracts/_.contract.test.ts" to:
...
- 7 contract tests verify data handoff points (TEST-01)
