...
function checkReadmeOnboarding(rootDir, findings) {
  ...
  if (readmeContent && claudeContent) {
    // Check if README mentions different major technologies than CLAUDE.md
    // This is a lightweight check - detects if both docs agree on stack components
    const readmeHasFramework = /(?:next\.?js|nuxt|remix|angular|vue|svelte)/i.test(readmeContent);
    const claudeHasFramework = /(?:next\.?js|nuxt|remix|angular|vue|svelte)/i.test(claudeContent);
    ...
    if (readmeHasFramework !== claudeHasFramework || readmeHasReact !== claudeHasReact) {
      consistencyOk = false;
    }
  }
  if (consistencyOk) {
...
function identifyMajorSystems(rootDir) {
  ...
  const majorDirs = [
    { dir: 'src', name: 'Application source', searchTerms: ['src/', 'source code', 'app/'] },
    {
      ...
      searchTerms: ['functions/', 'serverless', 'api endpoint'],
    },
