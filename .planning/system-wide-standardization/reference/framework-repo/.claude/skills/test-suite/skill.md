---
name: test-suite
description: Multi-phase UI testing orchestration using Playwright MCP or Chrome Extension
---
...
Multi-phase UI testing orchestration. Runs smoke tests, feature protocol tests,
security checks, and performance checks against any deployment URL using
...
3. Production URL fallback (configure in project)
...
3. Navigate to each configured route
4. On each page:
...
```json
{
  ...
  "status": "pass",
  "duration_ms": 850,
  "assertions": [{ "type": "text_visible", "value": "App Title", "passed": true }],
...
1. Navigate to protected routes without auth → verify redirect to login
