...
function npmScriptExists(pkgJson, scriptName) {
  ...
  // Handle sub-command args: "session:gaps -- --apply" → check "session:gaps"
...
function checkSessionDocFreshness(rootDir, findings) {
  ...
  if (lineCount < 300) {
    ...
    findings.push({
      ...
      details:
        'Large session context slows down AI loading. Archive older summaries to a SESSION_HISTORY file.',
      impactScore: 45,
      frequency: 1,
      blastRadius: 2,
      patchType: 'doc_trim',
      patchTarget: 'SESSION_CONTEXT.md',
      patchContent: 'Move older session summaries to an archive file',
  ...
  if (nextGoalsIdx !== -1) {
    ...
    if (goalsLines.length >= 1) {
      ...
      findings.push({
        ...
        patchType: 'doc_update',
        patchTarget: 'SESSION_CONTEXT.md',
        patchContent: 'Populate Next Session Goals with priority items',
  ...
  // Generic pattern: **Session #NNN Summary** or **Session Summary**
  ...
  if (summaryCount <= 3) {
    ...
    findings.push({
      ...
      details: 'Excess summaries bloat context. Archive older ones to a history file.',
      impactScore: 40,
      frequency: 1,
      blastRadius: 2,
      patchType: 'doc_trim',
      patchTarget: 'SESSION_CONTEXT.md',
      patchContent: `Archive ${summaryCount - 3} older session summaries`,
