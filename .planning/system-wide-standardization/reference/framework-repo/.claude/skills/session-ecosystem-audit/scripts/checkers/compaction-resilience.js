...
function checkLayerACommitTracker(rootDir, hooksSection, findings) {
  ...
  if (trackerContent.length > 0) {
    ...
    findings.push({
      ...
      details:
        'Expected file at .claude/hooks/commit-tracker.js. Layer A cannot function without this file.',
      impactScore: 95,
      frequency: 1,
      blastRadius: 5,
      patchType: 'create_file',
      patchTarget: '.claude/hooks/commit-tracker.js',
      patchContent: 'Create commit-tracker.js to log commits to .claude/state/commit-log.jsonl',
...
function checkLayerCPrecompactSave(rootDir, hooksSection, findings) {
  ...
  if (saveContent.length > 0) {
    ...
    findings.push({
      ...
      patchContent: 'Create pre-compaction-save.js to snapshot state before context compaction',
...
function checkLayerDGapDetection(rootDir, findings) {
  ...
  if (gapContent.length > 0) {
    ...
    findings.push({
      ...
      patchContent: 'Create check-session-gaps.js to detect undocumented sessions',
...
function checkRestoreOutputQuality(rootDir, hooksSection, findings) {
  ...
  if (restoreContent.length > 0) {
    ...
    findings.push({
      ...
      patchContent: 'Create compact-restore.js to output recovery context after compaction',
