 ...
 *   node run-pr-ecosystem-audit.js --batch   # Batch mode (suppress state writes)
 *   node run-pr-ecosystem-audit.js --save-baseline  # Save current scores as baseline
 *
...
if (!isBatchMode) {
  ...
  console.error('  [state] Batch mode — skipping state write');
}

// Save baseline if requested
if (isSaveBaseline) {
  const baselineSaved = stateManager.saveBaseline({
    timestamp: result.timestamp,
    healthScore: result.healthScore,
    categories: categoriesOutput,
    summary: result.summary,
  ...
  if (baselineSaved) {
    console.error('  [baseline] Saved baseline snapshot');
  } else {
