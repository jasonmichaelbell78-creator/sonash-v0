...
const PATCH_TYPES = {
  ...
  review_tool_config: {
    description: 'Update review tool suppression config',
    action: 'edit',
  },
  skill_update: {
...
function createPatchGenerator(rootDir) {
  ...
  function generate(finding) {
    ...
    switch (finding.patchType) {
      ...
      case 'review_tool_config':
        return generateReviewToolConfigPatch(finding);
      case 'skill_update':
  ...
  /**
   * Generate a review tool config patch.
   */
  function generateReviewToolConfigPatch(finding) {
    return {
      type: 'review_tool_config',
      description: `Update review tool suppression: ${finding.message}`,
      target: finding.patchTarget || '.review-tool/config.toml',
