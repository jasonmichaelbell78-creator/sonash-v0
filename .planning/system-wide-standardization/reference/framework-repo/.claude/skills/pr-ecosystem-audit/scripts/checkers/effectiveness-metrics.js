...
function checkAgentUtilization(reviewsJsonl, learnings, findings) {
  ...
  // Calibrated: multi-tool reviews ARE parallel agent/tool processing.
  // Detect both explicit agent mentions and multi-tool evidence.
  const agentKeywords = [
    'parallel agent',
    'code-reviewer agent',
    'specialist agent',
    ...
    'agent',
    'multi-agent',
    'static analysis', // automated tool = agent in multi-tool workflow
