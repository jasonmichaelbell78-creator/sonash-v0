...
function createStateManager(rootDir, isSafeToWrite) {
  ...
  const BASELINE_FILE = path.join(STATE_DIR, 'pr-ecosystem-audit-baseline.json');
  ...
  /**
   * Save current scores as a baseline snapshot.
   * @param {object} snapshot - The baseline data to save
  ...
  /**
   * Load previously saved baseline snapshot.
