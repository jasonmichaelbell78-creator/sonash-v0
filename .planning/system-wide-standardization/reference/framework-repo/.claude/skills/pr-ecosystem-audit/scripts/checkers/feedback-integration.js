...
function checkExternalToolConfiguration(rootDir, reviewsJsonl, findings) {
  ...
  // Check for review tool configuration files
  // Support multiple common review tool config patterns
  const configPaths = [
    { path: path.join(rootDir, '.review-tool', 'config.toml'), name: 'review-tool/config.toml' },
    { path: path.join(rootDir, '.github', 'review-config.yml'), name: 'github/review-config.yml' },
  ];
  ...
  for (const cfg of configPaths) {
    try {
      if (fs.existsSync(cfg.path)) {
        ...
        if (!configSections.reviewer) {
          configSections.reviewer =
            content.includes('[pr_reviewer]') || content.includes('reviewer:');
        }
        if (!configSections.code_suggestions) {
          configSections.code_suggestions =
            content.includes('[pr_code_suggestions]') || content.includes('code_suggestions:');
        }
        if (!configSections.compliance_checker) {
          configSections.compliance_checker =
            content.includes('[pr_compliance_checker]') || content.includes('compliance_checker:');
        }
  ...
  const configuredSections = Object.values(configSections).filter(Boolean).length;
  const totalSections = Object.keys(configSections).length;
  ...
  if (configCompleteness < 100) {
    const missingSections = Object.entries(configSections)
      .filter(([, v]) => !v)
      .map(([k]) => k);
    ...
    findings.push({
      ...
      domain: DOMAIN,
      severity: 'warning',
      message: `Review tool config missing ${totalSections - configuredSections} section(s): ${missingSections.join(', ')}`,
      details:
        'All 3 review tool sections (reviewer, code_suggestions, compliance_checker) need suppression rules to reduce noise.',
      impactScore: 55,
      frequency: 1,
      blastRadius: 2,
      patchType: 'review_tool_config',
      patchContent: missingSections.map((s) => `[${s}]\n# Add suppression rules here`).join('\n\n'),
      patchImpact: 'Complete review tool configuration to suppress known false positives',
    });
  }
  ...
  return {
    score: avgScore,
    rating: avgScore >= 90 ? 'good' : avgScore >= 70 ? 'average' : 'poor',
    metrics: { configCompleteness, suppressionEffectiveness, staleRules, configSections },
  };
}
