...
function checkPatternEnforcementCoverage(rootDir, patternChecker, reviewsJsonl, findings) {
  ...
  if (falsePositivePct > bench.false_positive_pct.average) {
    findings.push({
      ...
      details:
        'High false positive rates waste review cycles. Consider updating review tool suppression config or pattern rules.',
      impactScore: 55,
  ...
  if (!autoUpdate.autoUpdated && consolidationJson && consolidationJson.consolidationNumber > 0) {
    findings.push({
      ...
      details:
        ...
        'Check that run-consolidation.js includes appendToCodePatterns().',
      impactScore: 50,
      frequency: 1,
      blastRadius: 2,
    });
