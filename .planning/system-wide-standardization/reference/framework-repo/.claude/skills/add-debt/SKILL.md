...
| ------------- | -------- | ----------------------------------------------- | -------------------------- |
| `file`        | Yes      | File path (relative to repo root)               | `src/auth/login.ts`        |
...
node scripts/debt/intake-manual.js \
  ...
  --category code-quality \
  --effort E1 \
  --description "Module lacks error boundary, crashes propagate to parent"
```
...
```bash
# Run for each item
node scripts/debt/intake-pr-deferred.js --pr 325 --file "file1.ts" ...
node scripts/debt/intake-pr-deferred.js --pr 325 --file "file2.ts" ...
```
