...
STEP 1: Run the external link checker script if it exists:
...
STEP 1: Run the content accuracy checker if it exists:
...
STEP 1: Run the header check script on all docs if it exists:
  node scripts/check-doc-headers.js --all 2>&1

STEP 2: For each document missing required header fields, determine if the
...
STEP 3: For each finding, use this schema.

Severity mapping:
- Missing header on important doc: S1
- Missing header on other doc: S2
...
Each line must be valid JSON with these fields:
{
  ...
  "acceptance_tests": ["Header check passes"],
  "auto_fixed": true|false,
  "agent": "1B",
  "wave": 1,
  "finding_type": "issue"
...
STEP 4: Check DOCUMENTATION_INDEX.md accuracy if it exists:
...
Severity mapping:
- Orphaned important doc: S1 (important doc unreachable)
- Orphaned other doc: S2
- Missing from index: S2
...
STEP 2: Apply per-tier staleness thresholds (days since lastUpdated):
...
STEP 3: Check for deprecated references still in use:
...
STEP 5: Check for stray artifacts:
- .bak files in docs/
- .tmp files
- Files in root that belong in docs/ subdirectories

Severity mapping:
- Important doc stale >60 days: S1
- Other doc stale >90 days: S2
...
STEP 1: Run the document sync checker if it exists:
  node scripts/check-document-sync.js 2>&1

STEP 2: Run the cross-doc dependency checker if it exists:
...
STEP 4: Check template-instance sync if applicable:
...
Severity mapping:
- Unresolved [TODO] in important doc: S1
- Unresolved [TODO] in other doc: S2
...
Each line must be valid JSON with these fields:
{
  ...
  "suggested_fix": "specific fix",
  "acceptance_tests": ["Cross-doc check passes"],
  "auto_fixed": false,
  "agent": "2D",
  "wave": 2,
  "finding_type": "issue"
...
STEP 2: Read all important/canonical documents fully. Sample other documents
(read first 200 lines of each).
...
STEP 4: Track terminology consistency. Check for inconsistent usage of project-
specific terms and naming conventions.
...
- Terminology inconsistency in canonical docs: S2
- Terminology inconsistency in other docs: S3
...
STEP 2: Review documentation standards if a DOCUMENTATION_STANDARDS.md exists.
...
- Docs that are effectively canonical but not recognized as such
- Docs in wrong directories
- Docs that should be promoted or demoted

Severity mapping:
- Important doc misclassified: S2
...
STEP 2: For each important document (fully read) and a sample of other
...
- Glob src/**/*.ts or similar -> check if key modules have docs
...
- Start from any DOCUMENTATION_INDEX.md -> verify all categories reachable
...
STEP 6: Evaluate any documentation index:
