...
try {
  ...
  // Sanitize error: only expose error code, not full message
...
function checkSecurity() {
  ...
  if (fs.existsSync(encryptedPath)) {
    ...
    try {
      ...
      hasValidTokens =
        /CI_TOKEN=(?!your-|placeholder|xxx)[^\s]+/.test(envContent) ||
...
function checkAgentCompliance() {
  ...
  const securityFiles = changedFiles.filter((f) => /security|auth|\.env|\.config\.\w+/i.test(f));
...
function checkSkipAbuse() {
  ...
  const WINDOW_SIZE = (7 * DAY_MS) / WINDOW_COUNT;
  ...
  for (const e of last7d) {
    ...
    windowCounts[WINDOW_COUNT - 1 - windowIdx]++;
  }
...
function checkTestResults() {
  ...
  addContext('test-results', {
    ...
    ratings: { pass_rate: passRateRating, staleness: stalenessRating },
    totals: {
      passed,
      failed,
      errored,
...
function checkReviewQuality() {
  ...
  addContext('review-quality', {
    ...
    ratings: { fix_ratio: fixRatioRating, rounds: roundsRating },
    totals: {
...
function checkVelocity() {
  ...
  addContext('velocity', {
    ...
    acceleration,
    totals: { avg_items_per_session: Math.round(avg * 10) / 10, recent_sessions: recent.length },
  });
}
...
function checkRoadmapValidation() {
  ...
  addContext('roadmap-health', { totals: { errors: errorCount, warnings: warningCount } });
}
