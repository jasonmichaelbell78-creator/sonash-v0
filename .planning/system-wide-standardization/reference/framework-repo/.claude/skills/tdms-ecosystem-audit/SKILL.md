...
description: |
  ...
  and interactive finding-by-finding walkthrough. Covers TDMS scripts, data files,
  pipeline integrity, and reporting accuracy.
---
...
7. **Defer findings** by logging them to the deferred list for later review
...
1. Log the finding to the deferred list with:
   - severity: S1 (errors) or S2 (warnings)
   - category: engineering-productivity
   - source: "review:tdms-ecosystem-audit-{date}"
...
| Intake Pipeline        | All intake scripts produce valid format, write to BOTH master and deduped files |

### Domain 2: Data Quality & Deduplication (25% weight)

| Category                      | What It Checks                                                        |
| ----------------------------- | --------------------------------------------------------------------- |
| Dedup Algorithm Health        | Multi-pass dedup accuracy, merge relationships, cluster consistency   |
...
| Category                | What It Checks                                                                 |
| ----------------------- | ------------------------------------------------------------------------------ |
| Error Handling Coverage | All scripts wrap file reads in try/catch; JSON.parse protected                 |
| Master-Deduped Sync     | MASTER_DEBT.jsonl and raw/deduped.jsonl in sync                                |
...
| Source         | Path                                     | Content                                  |
| -------------- | ---------------------------------------- | ---------------------------------------- |
| TDMS scripts   | `scripts/debt/*.js`                      | Pipeline and utility scripts             |
| Canonical data | `docs/technical-debt/MASTER_DEBT.jsonl`  | Technical debt items ledger              |
...
Discovery (extract-*.js)
  → Intake (intake-*.js)
    → Normalization (normalize-all.js)
      → Deduplication (dedup-multi-pass.js, multi-pass)
        → Views (generate-views.js)
          → Metrics (generate-metrics.js)

Critical invariant: MASTER_DEBT.jsonl == raw/deduped.jsonl
  (generate-views.js reads deduped, writes to master — keep both in sync)
```
