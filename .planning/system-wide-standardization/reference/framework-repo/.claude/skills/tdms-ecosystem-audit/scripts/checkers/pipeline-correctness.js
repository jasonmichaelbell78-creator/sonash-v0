 ...
 * 3. Intake Pipeline — dual-write compliance
 */
...
const PIPELINE_PHASES = [
  {
    id: 'extract',
    label: 'Extract',
    patterns: ['extract-audits', 'extract-reviews'],
  },
 ...
 */
const INTAKE_SCRIPTS = ['intake-audit.js', 'intake-manual.js', 'intake-pr-deferred.js'];
 ...
 * 1. Writes to BOTH MASTER_DEBT.jsonl AND raw/deduped.jsonl
...
function checkIntakePipeline(rootDir, findings) {
  ...
  for (const scriptName of INTAKE_SCRIPTS) {
    ...
    // Check 2: Writes to raw/deduped.jsonl (dual-write requirement)
    ...
    if (!writesDeduped) {
      missingBehaviors++;
      scriptIssues.push('does not write to raw/deduped.jsonl');
    }
    ...
    if (scriptIssues.length > 0) {
      issues.push({
        ...
        severity: scriptIssues.some((i) => i.indexOf('deduped') !== -1) ? 'error' : 'warning',
      });
    }
  }
  ...
  for (const item of issues) {
    findings.push({
      ...
      patchImpact: 'Prevents generate-views.js from overwriting MASTER_DEBT.jsonl with stale data',
    });
  }
