...
function checkMasterDedupedSync(rootDir, debtScripts) {
  ...
  if (generateViewsScript) {
    ...
    if (generateViewsScript.content.includes('deduped.jsonl')) {
      ...
      findings.push({
        ...
        details: 'generate-views.js must read from raw/deduped.jsonl',
        impactScore: 85,
        frequency: 1,
        blastRadius: 5,
      });
  ...
  // Check intake scripts write to BOTH files
  ...
  for (const scriptName of intakeScriptNames) {
    ...
    if (writesMaster && writesDeduped) {
      ...
      findings.push({
        ...
        details: 'Intake scripts must write to BOTH files to prevent data loss on generate-views',
        impactScore: 90,
        frequency: 1,
        blastRadius: 5,
      });
