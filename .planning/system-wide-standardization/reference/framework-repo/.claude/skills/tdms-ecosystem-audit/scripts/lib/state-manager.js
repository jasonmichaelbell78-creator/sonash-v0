...
/**
 * State manager for TDMS Ecosystem Audit.
 *
 * Handles reading/writing .claude/state/tdms-ecosystem-audit-history.jsonl,
...
function createStateManager(rootDir, isSafeToWrite) {
  ...
  const STATE_FILE = path.join(STATE_DIR, 'tdms-ecosystem-audit-history.jsonl');
  const BASELINE_FILE = path.join(STATE_DIR, 'tdms-audit-baseline.json');
  ...
  function saveBaseline(entry) {
    try {
      ...
      if (!isSafeToWrite(BASELINE_FILE)) return false;
      fs.writeFileSync(BASELINE_FILE, JSON.stringify(entry, null, 2), 'utf8');
      return true;
    } catch {
      return false;
    }
