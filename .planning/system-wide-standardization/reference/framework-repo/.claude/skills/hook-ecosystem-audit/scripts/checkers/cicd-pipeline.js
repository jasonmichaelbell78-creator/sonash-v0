...
function checkWorkflowScriptAlignment(rootDir, findings) {
  ...
  if (invalidRefs.length > 0) {
    const details = invalidRefs
      ...
      .map(
        (r) =>
          `${r.workflow}:L${r.line} — ${r.type === 'missing_script' ? 'npm run ' : ''}${r.ref}`,
      )
      .join('; ');
    findings.push({
      id: 'HEA-601',
...
const BOT_CONFIG_PATTERNS = [
  ...
  {
    name: 'CodeQL',
    patterns: ['.github/codeql-config.yml', '.github/codeql-config.yaml'],
  },
  {
    name: 'CodeRabbit',
    patterns: ['.coderabbit.yml', '.coderabbit.yaml', 'coderabbit.yml'],
  },
];
...
function checkBotConfigFreshness(rootDir, findings) {
  ...
  for (const bot of BOT_CONFIG_PATTERNS) {
    ...
    if (found) {
      ...
      const healthIssues = checkBotConfigHealth(bot.name, configContent);
  ...
  if (missingBots.length > 0) {
    findings.push({
      ...
      details:
        'Consider adding bot configurations for automated code review and dependency updates.',
      impactScore: 25,
 ...
 */
function checkBotConfigHealth(botName, content) {
  const issues = [];
