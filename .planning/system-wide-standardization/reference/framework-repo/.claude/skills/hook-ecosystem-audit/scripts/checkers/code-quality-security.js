...
/**
 * Get the main hook files (top-level .js files, excluding lib/ and global/).
  ...
  for (const hook of hookFiles) {
    ...
    if (hasErrorLogging && !usesSanitize) {
      findings.push({
        ...
        details:
          'Consider using a sanitize-error helper for error output to avoid leaking internals',
        impactScore: 30,
        frequency: 1,
        blastRadius: 1,
      });
...
function checkSecurityPatterns(hookFiles) {
  ...
  for (const hook of hookFiles) {
    ...
    // Check 4: $ARGUMENTS usage — hooks receiving external input should validate
...
function checkCodeHygiene(hookFiles, rootDir) {
  ...
  // Collect documentation files to check for stale hook references
  const docFiles = [];

  // Check .claude/ docs
  const claudeDocsDir = path.join(rootDir, '.claude', 'docs');
  if (fs.existsSync && fs.existsSync(claudeDocsDir)) {
    try {
      safeReadDir(claudeDocsDir)
        .filter((f) => f.endsWith('.md'))
        .forEach((f) => {
          const content = safeReadFile(path.join(claudeDocsDir, f));
          if (content) docFiles.push({ name: f, filePath: path.join(claudeDocsDir, f), content });
        });
    } catch {
      // ignore
    }
  ...
  // Check root-level docs if present
...
function checkRegexSafety(hookFiles) {
  ...
  for (const hook of hookFiles) {
    ...
    for (const litMatch of hook.content.matchAll(regexLiteralPattern)) {
      ...
      regexInstances.push({ pattern, flags, lineNum, raw: litMatch[0].trim(), source: 'literal' });
    }
