 ...
 *
 * 11. Test Coverage — how many hooks have test cases
 ...
 * Discovers dynamically rather than using a hardcoded list.
 * @param {string} hooksDir
 * @returns {string[]}
 */
function getHookInventory(hooksDir) {
  const _EXCLUDED_DIRS = new Set(['lib', 'global', 'backup', 'node_modules']);
  try {
    const entries = fs.readdirSync(hooksDir, { withFileTypes: true });
    return entries
      .filter((e) => {
        if (!e.isFile()) return false;
        if (!e.name.endsWith('.js')) return false;
        return true;
      })
      .map((e) => e.name);
  } catch {
...
/**
 * Find test files for hooks in common locations.
 * @param {string} rootDir
 * @returns {Array<{ path: string, content: string }>}
 */
function findHookTestFiles(rootDir) {
  const candidatePaths = [
    path.join(rootDir, 'scripts', 'test-hooks.js'),
    path.join(rootDir, '.claude', 'hooks', 'tests'),
    path.join(rootDir, 'test', 'hooks'),
    path.join(rootDir, 'tests', 'hooks'),
    path.join(rootDir, '__tests__', 'hooks'),
  ];
...
/**
 * Extract hook filenames that have test entries in test files.
 * @param {Array<{ content: string }>} testFiles
 * @param {string[]} hookInventory
 * @returns {Set<string>}
 */
function extractTestedHooks(testFiles, hookInventory) {
  const tested = new Set();
  const allTestContent = testFiles.map((f) => f.content).join('\n');

  if (!allTestContent) return tested;

  // Match object keys like "commit-tracker.js": { or 'hook-name.js': {
  const keyRegex = /["']([a-zA-Z0-9_-]+\.js)["']\s*:\s*\{/g;
  for (const match of allTestContent.matchAll(keyRegex)) {
    tested.add(match[1]);
  }

  // Also check for describe() blocks or test names referencing hook filenames
  for (const hookName of hookInventory) {
    ...
    if (refRegex.test(allTestContent)) {
      tested.add(hookName);
    }
  }
...
/**
 * Check which hooks have test cases in hook test files.
 * @param {string} rootDir
 * @param {string[]} hookInventory
...
function checkTestCoverage(rootDir, hookInventory, findings) {
  const testFiles = findHookTestFiles(rootDir);
  ...
  if (testFiles.length === 0) {
    findings.push({
      ...
      domain: DOMAIN,
      severity: 'warning',
      message: 'No hook test files found',
      details:
        'Expected test files in scripts/test-hooks.js, .claude/hooks/tests/, test/hooks/, or tests/hooks/.',
      impactScore: 60,
      frequency: 1,
      blastRadius: 4,
    });
  ...
  const testedHooks = extractTestedHooks(testFiles, hookInventory);
...
function checkOutputProtocol(hooksDir, hookInventory, findings) {
  ...
  // Files that are utility modules, not event hooks — exempt from "ok" output requirement
  const UTILITY_MODULES = new Set(['state-utils.js', 'utils.js', 'helpers.js']);
  ...
  for (const hookFile of hookInventory) {
    ...
    if (!hasOkOutput && !UTILITY_MODULES.has(hookFile)) {
      hookIssues.push("missing 'ok' output for success path");
    }

    // Check 2: process.exit() usage
    ...
    // Check 3: Blocking capability — hooks with process.exit(1) should have block messaging
...
function checkBehavioralAccuracy(rootDir, hooksDir, hookInventory, findings) {
  ...
  const unregisteredHooks = hookInventory.filter(
    (h) => !registeredFiles.has(h) && !UTILITY_MODULES.has(h),
  );
  if (unregisteredHooks.length > 0) {
    ...
    findings.push({
      ...
      details:
        ...
        `These may be utility modules loaded by other hooks or unused files.`,
      impactScore: 20,
