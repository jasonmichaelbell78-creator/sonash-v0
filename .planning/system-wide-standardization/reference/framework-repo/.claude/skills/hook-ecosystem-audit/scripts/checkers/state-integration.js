    ...
    for (const entry of entries) {
      ...
      if (lineCount > 1000) {
        findings.push({
          ...
          details: 'File may need rotation. Consider implementing a state rotation mechanism.',
          impactScore: 40,
          frequency: 1,
          blastRadius: 2,
        });
 ...
 *   Layer A: A commit-tracking hook exists and writes to a commit log state file
 *   Layer B: A pre-compaction-save hook exists and creates snapshots
 *   Layer C: PreCompact hooks registered in settings.json
 *   Layer D: A compact-restore hook exists and registered for SessionStart:compact
 */
// eslint-disable-next-line complexity -- audit checker with many branches
function checkCompactionResilience(rootDir, hooksDir, findings) {
  ...
  // ── Layer A: commit-tracker writes to commit-log state file ─────────────
  ...
  try {
    if (fs.existsSync(commitTrackerPath)) {
      ...
      if (writesCommitLog) {
        layerStatus.A = { present: true, details: 'commit-tracker.js writes to commit-log state' };
        layersPresent++;
      } else {
        layerStatus.A = {
          present: false,
          details: 'commit-tracker.js exists but does not reference commit-log state',
        };
        findings.push({
          id: 'HEA-520',
          ...
          domain: DOMAIN,
          severity: 'warning',
          message: 'Layer A: commit-tracker.js does not reference commit-log state',
          details:
    ...
    if (fs.existsSync(preCompactionPath)) {
      ...
      if (createsSnapshot) {
        ...
        findings.push({
          ...
          details: 'The pre-compaction hook exists but may not be saving state before compaction.',
          impactScore: 55,
          frequency: 1,
          blastRadius: 3,
        });
