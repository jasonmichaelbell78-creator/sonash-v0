...
### Agent 1E: Backend Services Inventory

```
Task(subagent_type="Explore", prompt="""
Inventory backend service automation (adapt to what exists in this project):

1. Backend functions / serverless functions (if any) - list each with type and purpose
2. Scheduled jobs or cron tasks (if any) - document schedule and purpose
3. Infrastructure rule files (security rules, access policies, etc. if any)
4. Backend configuration indexes or manifests (if any)

For each item found, document:
- File path
- Type (callable, scheduled, trigger, rule, etc.)
- Schedule (if applicable)
- What it does

If none of these exist in this project, note "No backend automation found" and
document what infrastructure patterns ARE present.

CRITICAL: You MUST write your findings directly to this file:
  ${AUDIT_DIR}/stage-1e-backend.md
...
5. Backend functions not referenced anywhere (if applicable)
...
4. Scheduled jobs that are disabled (if applicable)
...
4. Backend functions without integration tests (if applicable)
