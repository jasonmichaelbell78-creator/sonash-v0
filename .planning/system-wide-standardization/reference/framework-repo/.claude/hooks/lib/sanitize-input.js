 ...
 * input cleaning across hook scripts that write to JSONL state files.
 */
...
function sanitizeInput(value, maxLength = 500) {
  ...
  for (let i = 0; i < value.length && i < maxLength * 2; i++) {
    ...
    // and Unicode characters (128+) to preserve international text
    if ((code >= 32 && code <= 126) || code >= 128 || code === 9 || code === 10 || code === 13) {
      sanitized += value[i];
    }
  }
