 ...
 * eslint-plugin-framework. This file retains only patterns that:
 ...
 * - unsafe-error-message → no-unsafe-error-access
 * - path-startswith → no-path-startswith
 * - hardcoded-api-key → no-hardcoded-secrets
 * - unsafe-innerhtml → no-unsafe-innerhtml
 * - eval-usage → ESLint no-eval
 * - unstable-list-key → no-index-key
 * - div-onclick-no-role → no-div-onclick-no-role
 * - sql-injection-risk → no-sql-injection
 * - shell-command-injection → no-shell-injection
 * - sql-injection-template → no-sql-injection
 */
