...
function isSafeToWrite(filePath) {
  try {
    ...
    // Check the file itself (single lstatSync call to avoid TOCTOU race)
    try {
      if (fs.lstatSync(filePath).isSymbolicLink()) return false;
    } catch (e) {
      if (e.code !== 'ENOENT') return false; // fail closed on unexpected errors
    }
    ...
    while (true) {
      try {
        ...
        if (e.code !== 'ENOENT') return false; // fail closed
      }
