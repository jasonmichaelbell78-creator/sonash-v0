 ...
 * exactly the right moment — unlike threshold-based triggers which
 * may miss sessions with unusual tool usage patterns.
 *
 * Data captured:
 ...
 */
// eslint-disable-next-line complexity -- gatherGitContext has inherent branching (complexity 20), refactoring would reduce readability
function gatherGitContext() {
  ...
  // Call 3: git status --porcelain -z (NUL-separated for filenames with spaces)
  ...
  for (let i = 0; i < statusFields.length; i++) {
    ...
    // Renames/copies have an extra NUL-separated path
...
/**
 * Detect active audits by scanning for progress files
 */
 ...
 * Main
 */
// eslint-disable-next-line complexity -- main has inherent branching (complexity 22), refactoring would reduce readability
function main() {
  ...
  const handoff = {
    ...
    recovery: {
      instruction:
        ...
        (activeAudits
          ? `activeAudits (${activeAudits.length} audit(s) in progress — resume with their /skill commands). `
          : '') +
