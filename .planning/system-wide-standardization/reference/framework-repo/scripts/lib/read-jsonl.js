...
function readJsonl(filePath, options = {}) {
  ...
  try {
    ...
    console.error(
      `Failed to read ${path.basename(filePath)}: ${err.code || (err instanceof Error ? err.message : String(err))}`, // eslint-disable-line framework/no-unsafe-error-access -- safe: instanceof guard is in conditional expression
    );
    process.exit(1);
  }
