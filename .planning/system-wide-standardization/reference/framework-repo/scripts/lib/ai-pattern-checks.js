 ...
 * @returns {{ exists: boolean, type: string|null }} Import status
 */
// eslint-disable-next-line complexity -- checkImportExists has inherent branching (complexity 26), refactoring would reduce readability
function checkImportExists(importName, packageJsonPath = 'package.json') {
  ...
  // Built-in Node.js modules - use canonical list
  ...
  // Using regex to avoid CI pattern check false positives
  if (/^\./.test(importName)) {
...
function extractImports(content) {
  ...
  // Re-exports (also import a module specifier)
  const reExportRegex =
