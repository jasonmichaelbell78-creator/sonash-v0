...
function sanitizeFilesystemError(err) {
  ...
  // Strip Unicode line separators to prevent log injection
  ...
  // Prevent log flooding from unusually large errors
 ...
 */
// eslint-disable-next-line complexity -- validateFilePath has inherent branching (complexity 20), refactoring would reduce readability
function validateFilePath(filePath, projectDir) {
  // Fail closed on invalid projectDir
  if (typeof projectDir !== 'string' || !projectDir.trim()) {
    return { valid: false, error: 'Invalid project directory', normalized: null };
  }

  // Security: Cap input length to prevent DoS
  if (projectDir.length > 4096) {
    return { valid: false, error: 'Project directory path too long', normalized: null };
  }

  // Reject non-string paths early
  ...
  // Security: Reject control chars (defense-in-depth)
  ...
  // Use [0] instead of startsWith to avoid pattern trigger
  if (filePath[0] === '-') {
    return { valid: false, error: 'Option-like path rejected', normalized: null };
  }

  // Security: Reject multiline paths (prevent injection) - includes Unicode line separators
  if (
    filePath.includes('\n') ||
    filePath.includes('\r') ||
    filePath.includes('\u2028') ||
  ...
  // Allow absolute paths only if they are contained in projectDir
  if (path.isAbsolute(filePath)) {
    const abs = path.resolve(filePath);
    const rel = path.relative(projectRoot, abs);
    // Use segment-based check instead of startsWith
  ...
  // Security: Block path traversal using improved regex
...
function verifyContainment(filePath, projectDir) {
  // Defense-in-depth: Validate format first
  ...
  // Resolve symlinks without TOCTOU race
  ...
  try {
    ...
    // Sanitize other filesystem errors
    return {
      contained: false,
