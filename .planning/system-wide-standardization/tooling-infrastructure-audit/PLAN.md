...
`if grep -RIn -- '@v[0-9]' .github/workflows/ | grep -vE '^\s*#'; then echo "::error::Found unpinned actions using version tags. Please use SHA pins."; exit 1; fi`
...
3. Document findings in
   `.planning/system-wide-standardization/tooling-infrastructure-audit/npm-script-audit.md`
...
- Have broken internal links Document findings in
  `.planning/system-wide-standardization/tooling-infrastructure-audit/skill-orphan-scan.md`
