...
| D2  | Mode scope                  | Automated Assess + conversational Extract                                                              | Context quality > model capability. Repomix compressed context + value map primes extraction conversation at ~20% build cost (estimator assertion, not research-backed figure) |
...
| D18 | Repomix integration         | Run `repomix --compress` on clone, save to `.research/<slug>/repomix-output.txt`                       | Purpose-built for conversation-ready compressed context; ~70% token reduction (LOW confidence on specific %, directionally valid)                                              |
