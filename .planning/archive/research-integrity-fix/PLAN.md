...
**Topic:** Fix deep-research pipeline integrity + commit all research
artifacts + remediate existing outputs + verify implementations and plans
**Date:** 2026-04-02 **Decisions:** [DECISIONS.md](./DECISIONS.md) (21
decisions) **Research:** Audit conducted this session — 9 research outputs,
content-level analysis, dependency mapping **Effort:** L-XL (multi-session)
...
Per Decision #21 — raw agent output files have never been through linting or doc
headers. Run a formatting pass BEFORE committing to avoid pre-commit failures.

1. Inventory all files:
   `find .research/*/findings .research/*/challenges -type f`
...
**Depends on:** Step 0 **Done when:** All 172 files pass formatting checks.
...
4. Single commit: "fix: commit 172 gitignored research artifacts for data
   integrity"

**Depends on:** Step 1 **Done when:** `git status` shows no untracked files
under `.research/`.
...
- JSONL: `.claude/state/research-validation.jsonl` — one line per finding, feeds
  into TDMS/alerts
...
1. **github-health**: Fix agentCount (21→32), claimCount (38→100+), sourceCount
   (27→103+), confidenceDistribution, remove stale
   `findingsFilesCompleted`/`findingsFilesMissing`/`coverageNote` fields.
2. **custom-agents**: Fix agentCount (37→49), confidenceDistribution (MEDIUM
   32→35).
3. **dev-dashboard**: Fix agentCount (36→42 or verify report says 35+).
4. **repo-analysis-skill**: Fix agentCount (32→31 per PR),
   confidenceDistribution (HIGH 37→42, MEDIUM 10→6, remove LOW/UNVERIFIED that
   don't exist).
...
1. **github-health**: Add C-039 (CODEOWNERS finding from V1 verification). This
   is the worst case — claims.jsonl has 38 entries but report has 100. The final
   synthesizer needs to regenerate claims.jsonl from RESEARCH_OUTPUT.md claim
   registry. Consider: manually extract all ~39 claim IDs from report Section 5
   and ensure JSONL matches.
...
describes ~39 claim IDs (C-001 through C-039). The gap between 38/39 claims in
JSONL vs "100 claims" in the PR description suggests the PR counted differently
(possibly including prioritized issues P0-001 through P3-006 as "claims").
Verify the actual scope before rebuilding.
...
**Done when:** `npm run research:validate` shows source checks passing (or WARN
for accepted inconsistencies).
...
Pre-standard pipeline output (2026-03-24). No claims.jsonl or sources.jsonl. All
claims (8) and sources (100+) embedded in RESEARCH_OUTPUT.md narrative.
...
Single commit: "fix: remediate research metadata, claims, and sources across 8
outputs"

**Depends on:** Steps 6-9 **Done when:** `npm run research:validate` shows all
checks passing or WARN-only (no FAIL).
...
The final synthesizer MUST update these core fields by recounting from actual
artifacts:
...
If any check FAILs, the final synthesizer must fix it before completing. Checks
7-8 (source freshness, verdict persistence) are advisory only at pipeline time.
...
- Add explicit instruction: "Do NOT use research-phase codes (D1a, D2a-1) as
  source IDs. Map each source to a sequential S-### ID."
...
**Depends on:** Steps 11-13 **Done when:** Pipeline agents updated and
committed.
...
Focus on the 9 claims added during gap-pursuit (C-103 to C-111) that weren't in
the original JSONL.
...
**Code audit:** Read `.claude/skills/repo-analysis/SKILL.md` and `REFERENCE.md`.
Check whether any skill behavior references specific claims or findings that are
stale/wrong. The skill was built from the PLAN.md which references
RESEARCH_OUTPUT.md sections — check the chain.
...
Check: did the minor count discrepancies propagate into any execution decisions
in Waves 0-1b? Read the execution commits and verify.
...
**Depends on:** Step 18 + user approval **Done when:** All plan files updated.
Committed.
...
All 8 research outputs (excluding research-discovery-standard) should show PASS
on all 8 checks. Capture as "after" snapshot.
...
- **Steps 3-4** can start in parallel with Steps 1-2 (script doesn't depend on
  gitignore change)
