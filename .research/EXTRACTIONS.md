...
**Total:** 379 candidates across 35 sources | **By decision:** defer: 355, investigate: 2, extract: 20, skip: 2
...
| [Y2Z/monolith](#y2z-monolith-repo) | repo | 9 | 4 | 3 | 2 | 0 |
...
| Redlock distributed cache serialization | pattern | defer | 2026-04-10 | medium | E1 | low | - | Use redlock.using(\[key\], ttlMs, ...) to serialise cache updates across multiple pods. Prevents race conditions on shar |
...
| Hidden feature flags via env vars | anti-pattern | defer | 2026-04-10 | low | E0 | medium | - | Firecrawl branches behaviour through SELF_HOST.md env vars (USE_DB_AUTHENTICATION, PROXY_SERVER, SEARXNG_*) with no docu |
...
| Environment variable catalog | knowledge | defer | 2026-04-10 | low | E0 | high | - | docs/en/usage/cli_tools.md has prose catalog of 15+ MINERU_* env vars with default/effect/scoping. Template for docs/age |
...
<a id="y2z-monolith-repo"></a>

## Y2Z/monolith (repo)

| Candidate | Type | Decision | Date | Novelty | Effort | Relevance | Extracted To | Notes |
| --------- | ---- | -------- | ---- | ------- | ------ | --------- | ------------ | ----- |
| Per-module test mirror | pattern | defer | 2026-04-18 | low | E0 | medium | - | tests/<module>/ 1:1 mirrors src/<module>/ with colocated _data_/ fixtures. Removes where-does-this-test-live decision at |
| Optional feature flag for secondary binary | pattern | defer | 2026-04-18 | low | E1 | low | - | Cargo.toml optional deps + Makefile --features='gui' keeps default binary lean. Pattern for CLI-first / GUI-second witho |
| Strict dependency pinning (=X.Y.Z) | pattern | defer | 2026-04-18 | medium | E1 | medium | - | Cargo.toml pins every direct dep with = prefix. Candidate for SoNash meta-pipeline scripts (scripts/reviews, scripts/ski |
| Multi-channel distribution catalog | workflow-pattern | defer | 2026-04-18 | high | E2 | medium | - | 18 install channels (Cargo, Homebrew, Chocolatey, Scoop, Winget, MacPorts, Snap, Nix, Pacman, aports, XBPS, FreeBSD, pkg |
| Single-file archival as a content type | knowledge | defer | 2026-04-18 | high | E1 | high | - | Data-URL inlining of CSS/images/JS into one HTML5 document. Reference implementation when SoNash journal external-refere |
| CLI + managed-service wrapper (Apify Actor) | knowledge | defer | 2026-04-18 | low | E0 | medium | - | Same CLI wrapped as Apify Actor SaaS. Validates SoNash httpsCallable-over-deterministic-worker pattern. E0 - no change,  |
| CC0-1.0 public-domain licensing for tooling | knowledge | defer | 2026-04-18 | medium | E0 | low | - | Zero-friction extract/fork/embed permission. Informational for any SoNash Extract decision from this repo. |
| Pinning philosophy contradicted by Dockerfile | anti-pattern | defer | 2026-04-18 | medium | E0 | medium | - | Cargo.toml pins = everything but Dockerfile curls releases/latest unpinned. Lesson: supply-chain discipline must be unif |
| Mono-README without SECURITY.md | anti-pattern | defer | 2026-04-18 | medium | E0 | medium | - | 15k-star tool that fetches arbitrary web content ships no vuln-disclosure path. Lesson: SECURITY.md earns cost well befo |
