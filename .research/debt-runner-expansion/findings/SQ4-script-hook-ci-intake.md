...
Every script that creates new DEBT-XXXX entries writes to the same canonical files:
`docs/technical-debt/MASTER_DEBT.jsonl` (source of truth) and
...
All duplicate-check via content hash (SHA-256 of normalized file+line+title+desc)
before writing. Below is the full inventory:

#### 1a. `intake-audit.js`
- **Trigger:** Manual or via `/debt-runner` after a formal audit. Accepts a JSONL
  file argument.
...
- **Notable:** Only script that performs post-write pipeline processing including
  dedup and roadmap assignment. The audit-to-TDMS "funnel" script.

#### 1b. `intake-manual.js`
- **Trigger:** Manual invocation by Claude via `/add-debt` skill (no PR context).
  Direct CLI: `node scripts/debt/intake-manual.js --file ... --title ...`
...
- **Notable:** Sets both `pr_number` and `source_pr` fields. `source_id` format:
  ...
  when `defer_count >= threshold` (default 2) in `data/ecosystem-v2/deferred-items.jsonl`.
...
- **Output:** Writes `docs/technical-debt/raw/audits.jsonl` (full overwrite via
  `safeWriteFileSync`). Upstream feeder — must be followed by `ingest-cleaned-intake.js`.
...
- **Source data:** Two `.claude/state/` markdown files: `agent-research-results.md`
  and `system-test-gap-analysis-pass2.md`. Parses `Gap:` items and `FINDING-*`
  entries.
...
- **Notable:** The terminal step of the scattered-intake pipeline:
  `extract-*` → `clean-intake.js` → `ingest-cleaned-intake.js` → MASTER_DEBT.
...
overriding individual checks. Overrides are logged to `.claude/override-log.jsonl`
by `log-override.js`. `validate-skip-reason.js` enforces reason presence,
length, and character safety. No TDMS record is created for skipped checks.
...
Key observations:
- **`resolve-debt.yml` is the only CI workflow that writes to TDMS**, and it only
  resolves (never creates) items.
...
4. **`deferred-items.jsonl` vs MASTER_DEBT.jsonl are separate.** The ecosystem
   ...
   items with `defer_count >= 2`. Items deferred once never escalate automatically.
...
1. **Two distinct deferral queues co-exist.** `data/ecosystem-v2/deferred-items.jsonl`
   (ecosystem review deferrals, automatically escalated) and the conceptual
   "defer via /add-debt in /pr-review" path (manual). These converge in TDMS
   only through human action or the `escalate-deferred.js` auto-promotion.
   The distinction matters for `debt-runner` expansion: the expansion should
   consider whether it wants to be aware of both queues.
...
3. **The `alerts` skill is a secondary debt discovery path.** Via
   `checkDeferredItemsStaleness()` and the pending-refinements escalation
   logic, `/alerts` can surface actionable items that should become DEBT entries
   (e.g., S1 DEBT candidates from patterns surfaced many times). This is
   not documented in any "intake path" catalog but is a real production path.
