...
function mapErrorToUserMessage(error: AuthError): LinkError {
  ...
  const errorMap: Record<string, { message: string; recoverable: boolean }> = {
    "auth/credential-already-in-use": {
      message:
        "This email or Google account is already linked to another user. Try signing in instead.",
      recoverable: true,
    },
      ...
      try {
        ...
        return {
          ...
          error: {
            ...
            userMessage:
              "This email is already registered. Your anonymous data could not be merged. Please sign in directly.",
            recoverable: true,
          },
        };
      }
      ...
      try {
        ...
        return {
          ...
          error: {
            ...
            userMessage:
              "Unable to sign in. Please try refreshing the page and signing in directly.",
            recoverable: true,
          },
        };
      }
