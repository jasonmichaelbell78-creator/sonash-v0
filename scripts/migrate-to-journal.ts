  ...
  switch (type) {
    case "spot-check":
      ...
      if (Array.isArray(data.feelings))
        parts.push(...data.feelings.filter(isPrimitive).map(String));
      if (Array.isArray(data.absolutes))
        parts.push(...data.absolutes.filter(isPrimitive).map(String));
      break;
    case "night-review":
      ...
      if (
        data.step3_reflections &&
        typeof data.step3_reflections === "object" &&
        data.step3_reflections !== null
      ) {
