...
const KNOWN_PATTERN_RULES = [
  ...
  },
  {
    ...
    excludeFilePattern: /(^|[^A-Za-z0-9_$])(?:isSafeToWrite|isSymbolicLink|lstatSync|guardSymlink|refuseSymlink)\s*\(/,
    description: "writeFileSync without symlink check — use isSafeToWrite() or lstatSync + isSymbolicLink() guard",
    recommended: "Add isSafeToWrite(filePath) guard before writeFileSync, or use safe-fs.js helpers",
  },
  {
    name: "rmSync-usage",
    searchPattern: String.raw`(^|[^[:alnum:]_$])rmSync[[:space:]]*\(`,
    excludeFilePattern: null,
    description: "rmSync creates race condition risk with renameSync — prefer rename-only atomic patterns",
    recommended: "Use rename-only pattern or try { renameSync } catch { copyFileSync + unlinkSync } for cross-device",
  },
  {
    ...
    description: "Markdown table write without escapeCell — pipe chars in data corrupt table formatting",
  ...
  },
  {
    ...
    excludeFilePattern: null,
    description: ".filter(Boolean) treats 0 and '' as falsy — unsafe for nullable numbers or empty strings",
    recommended: "Use .filter(x => x != null) or .filter(x => x !== undefined) for type-safe filtering",
  },
];
