...
async function fetchSonarCloudData() {
  ...
  try {
    ...
    const issuesFetch = fetch(issuesUrl.toString(), fetchOptions);
    const hotspotsFetch = fetch(hotspotsUrl.toString(), fetchOptions);
    const gateFetch = fetch(gateUrl.toString(), fetchOptions);
