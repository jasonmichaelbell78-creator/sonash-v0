...
exports.generateFixTemplateStub = generateFixTemplateStub;
exports.appendFixTemplateStubs = appendFixTemplateStubs;
exports.main = main;
 ...
 */
function generateFixTemplateStub(pattern, templateNumber) {
    const displayName = pattern.pattern
        .replaceAll("-", " ")
        .replaceAll(/\b\w/g, (ch) => ch.toUpperCase());
    const prList = pattern.distinctPRs.size > 0
 ...
 */
function findNextTemplateNumber(content) {
    let maxNum = 0;
...
function isPatternAlreadyTemplated(lowerContent, patternName) {
    ...
    return (headingPattern.test(lowerContent) || lowerContent.includes(`**pattern:** ${normalizedName}`));
}
...
function writeFixTemplatesAtomic(fixTemplatesPath, existingContent, appendContent) {
    try {
        ...
        console.warn(`[generate-fix-template-stubs] Warning: Could not write FIX_TEMPLATES.md: ${err instanceof Error ? err.message : String(err)}`);
    }
}
/**
 ...
 */
function appendFixTemplateStubs(projectRoot, patterns, dryRun = false) {
