 ...
 */
function writeAtomicSafe(filePath, content) {
...
function buildSingleRetroRecord(retro, reviewsByPR) {
    ...
    const parsed = retro_1.RetroRecord.safeParse(candidate);
    if (!parsed.success) {
        console.warn(`Warning: Retro validation failed for PR #${retro.pr} (${retro.sourceFile}): ${parsed.error.message}`);
