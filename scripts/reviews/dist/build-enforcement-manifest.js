...
exports.parseCodePatterns = parseCodePatterns;
exports.scanRegexRules = scanRegexRules;
exports.scanEslintRules = scanEslintRules;
exports.scanSemgrepRules = scanSemgrepRules;
exports.buildMechanisms = buildMechanisms;
exports.slugify = slugify;
const fs = __importStar(require("node:fs"));
const path = __importStar(require("node:path"));
// Walk up from __dirname until we find package.json (forward-declared for isSafeToWrite)
function findProjectRootEarly(startDir) {
    let dir = startDir;
    for (;;) {
        try {
...
const { isSafeToWrite } = require(path.resolve(findProjectRootEarly(__dirname), "scripts/lib/safe-fs.js"));
 ...
 */
function parsePriority(emoji) {
    if (emoji.includes("\uD83D\uDD34") || emoji.includes("critical"))
        return "critical"; // red circle
    if (emoji.includes("\uD83D\uDFE1") || emoji.includes("important"))
        return "important"; // yellow circle
    return "edge";
}
/**
 * Parse CODE_PATTERNS.md to extract all patterns with their categories and priorities.
 */
function parseCodePatterns(filePath) {
    let content;
    try {
        content = fs.readFileSync(filePath, "utf-8");
    }
    catch (err) {
        throw new Error(`Failed to read CODE_PATTERNS.md: ${err instanceof Error ? err.message : String(err)}`);
    }
    const patterns = [];
 ...
 */
function scanRegexRules(projectRoot) {
    ...
    try {
        ...
        console.warn(`Warning: Could not scan regex rules: ${err instanceof Error ? err.message : String(err)}`);
    }
    return rules;
}
/**
 ...
 */
function scanEslintRules(projectRoot) {
    ...
    try {
        ...
        console.warn(`Warning: Could not scan ESLint rules: ${err instanceof Error ? err.message : String(err)}`);
    }
    return rules;
}
/**
 ...
 */
function scanSemgrepRules(projectRoot) {
 ...
 */
function scanClaudeMd(projectRoot) {
    ...
    try {
        ...
        console.warn(`Warning: Could not scan CLAUDE.md: ${err instanceof Error ? err.message : String(err)}`);
    }
    return patterns;
}
// --- Matching Logic ---
 ...
 */
function buildMechanisms(pattern, sources) {
    const mechanisms = {
        regex: "none",
        eslint: "none",
        semgrep: "none",
...
function main() {
    ...
    for (const pattern of patterns) {
        ...
        const coverage = (0, enforcement_manifest_1.classifyCoverage)(mechanisms);
        const record = {
        ...
        // Detect staleness
        if ((0, enforcement_manifest_1.isStale)(record)) {
            record.status = "stale";
        }
        records.push(record);
    }
    ...
    if (DRY_RUN) {
        ...
        for (const record of records) {
            (0, write_jsonl_1.appendRecord)(manifestPath, record, enforcement_manifest_1.EnforcementRecordSchema);
        }
