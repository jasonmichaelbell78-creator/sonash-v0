...
export function generateAntiPatternsTable(patterns: RecurrenceResult[], maxPatterns = 6): string {
  ...
  const rows = top.map((p) => {
    ...
    const patternRaw = String(p.pattern ?? "");
    const prCount = typeof p.distinctPRs?.size === "number" ? p.distinctPRs.size : 0;
    const name = escapeCell(
      patternRaw.replaceAll("-", " ").replaceAll(/\b\w/g, (ch) => ch.toUpperCase())
    );
    const rule = escapeCell(
      `Recurring (${p.count}x, ${prCount} PRs) -- review and add enforcement`
    );
    return `| ${name} | ${rule} |`;
  });
