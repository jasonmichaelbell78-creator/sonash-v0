 ...
 */
export function hasField(record: { completeness_missing?: string[] }, field: string): boolean {
...
export function validateCompleteness(
  ...
  for (const field of requiredFields) {
    if (!hasField(record, field)) {
      violations.push(
        `Field "${field}" is required for tier "${record.completeness}" but is in completeness_missing`
      );
    }
  }
