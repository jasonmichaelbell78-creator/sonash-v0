...
const QuestionBlock = ({ question, data, onUpdateArray, onUpdateField }: QuestionBlockProps) => {
  const { id, label, description, type, examplesField, resultsField, singleField, count } =
    question;
 ...
 */
export default function Step1WorksheetCard({
  className: _className,
  ...props
}: Step1WorksheetCardProps) {
      ...
      <Dialog open={isOpen} onOpenChange={handleOpenChange}>
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <div className="flex items-center justify-between">
              ...
              {!isLoading && !isSaving && hasContent && (
                <div
                  className="text-xs text-green-600/60 flex items-center gap-2"
                  role="status"
                  aria-live="polite"
                >
          ...
          <div className="space-y-6 py-4">
            <div
              className={`${currentSection.bgColor} border-l-4 ${currentSection.borderColor} p-4 rounded`}
            >
          ...
          <div className="flex items-center justify-between pt-4 border-t">
            ...
            <div className="flex flex-col items-center gap-2">
              <div className="text-sm text-amber-900/60">{section} / 4</div>
              <Button
                type="button"
                variant="ghost"
                size="sm"
