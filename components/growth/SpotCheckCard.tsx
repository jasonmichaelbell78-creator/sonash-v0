...
export default function SpotCheckCard({ className, ...props }: SpotCheckCardProps) {
  ...
  // Speech
  const { isListening, transcript, startListening, stopListening, resetTranscript, hasSupport } =
  ...
  const toggleAbsolute = (abs: string) => {
    setAbsolutes((prev) => (prev.includes(abs) ? prev.filter((a) => a !== abs) : [...prev, abs]));
  };
        ...
        <div className="bg-amber-100/50 px-6 py-4 flex justify-between items-center">
          {step > 1 ? (
            <Button
              ...
              className="text-amber-800 hover:text-amber-900 hover:bg-amber-200/50"
            >
              Back
            </Button>
          ) : (
