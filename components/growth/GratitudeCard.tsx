            ...
            <DialogContent className="sm:max-w-[500px] bg-[#fdfdfd] border-emerald-100 p-0 overflow-hidden text-emerald-950 flex flex-col max-h-[85vh]">
                <DialogHeader className="px-6 pt-6 pb-2 border-b border-emerald-100/50">
                    <DialogTitle className="font-handlee text-2xl text-emerald-800 flex items-center gap-2">
                        <span className="p-1.5 bg-emerald-100 rounded-md"><Heart className="w-4 h-4 text-emerald-600" /></span>
                        Gratitude List
                    </DialogTitle>
                </DialogHeader>
                ...
                <div className="flex-1 overflow-y-auto pl-6 pr-8 py-4 space-y-6">
                    ...
                    <div className="space-y-3 p-4 bg-emerald-50 rounded-xl border border-emerald-100">
                        ...
                        <div className="flex gap-2">
                            ...
                            />
                            {!showWhy && (
                                <Button size="icon" onClick={handleAddItem} disabled={!currentText.trim()} className="bg-emerald-600 hover:bg-emerald-700 text-white shrink-0">
                        ...
                        <AnimatePresence>
                            {showWhy && (
                                ...
                                >
                                    ...
                                    />
                                    <Button onClick={handleAddItem} disabled={!currentText.trim()} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white h-8 text-xs">
                                        Add to List
                                    </Button>
                                </motion.div>
                            )}
                                ...
                                items.map((item) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            {item.why && (
                                                <p className="text-xs text-emerald-900/60 italic mt-0.5 break-words">∵ {item.why}</p>
                                            )}
                                        </div>
                                        <button
                ...
                <div className="bg-emerald-50/50 border-t border-emerald-100 px-6 py-4 flex justify-between items-center">
                    <span className="text-xs text-emerald-900/40 font-handlee">{items.length} items logged</span>
                    <Button
                        onClick={handleSave}
