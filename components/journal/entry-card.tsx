  ...
  return (
    <motion.div
      ...
      animate={{
        opacity: 1,
        x: 0,
        rotate: entry.type === "mood" ? -2 : entry.type === "gratitude" ? 1 : 0,
      }}
      transition={{ delay: index * 0.1, type: "spring", stiffness: 100 }}
      onClick={onClick}
      className={cn(
        "relative mb-6 cursor-pointer",
        entry.type !== "mood" && entry.type !== "gratitude" && "w-full"
      )}
    >
      <div
        className={cn(
        ...
        {(entry.type === "free-write" || entry.type === "meeting-note") && (
          <div className="w-full">
            <div className="flex items-center justify-between mb-2">
              <h4 className="font-heading text-sm text-amber-900/80">
                {entry.data.title || "Note"}
              </h4>
            </div>
            ...
            <div className="flex flex-col gap-1.5 text-xs font-sans">
              {entry.data.cravings !== null && entry.data.cravings !== undefined && (
                <div className="flex items-center gap-2">
                  <span
                    className={`px-3 py-1.5 rounded-full font-medium ${entry.data.cravings ? "bg-amber-100 border border-amber-300 text-amber-800" : "bg-green-100 border border-green-300 text-green-800"}`}
                  >
              ...
              {entry.data.used !== null && entry.data.used !== undefined && (
                <div className="flex items-center gap-2">
                  <span
                    className={`px-3 py-1.5 rounded-full font-medium ${entry.data.used ? "bg-red-100 border border-red-300 text-red-800" : "bg-green-100 border border-green-300 text-green-800"}`}
                  >
