...
export function MeetingDetailsDialog({
  ...
  const getFullAddress = () => {
    const parts = [meeting.address, meeting.city, meeting.state, meeting.zip].filter(Boolean);
    return parts.join(", ");
  };
      ...
      <DialogContent className="sm:max-w-md bg-[#fdfbf7] border-amber-200">
        <DialogHeader>
          <DialogTitle className="font-heading text-2xl text-amber-900 flex items-center gap-2 pt-1">
            <div
              className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold border-2 shrink-0 ${meeting.type === "NA" ? "border-amber-500 text-amber-700 bg-amber-50" : "border-blue-400 text-blue-700 bg-blue-50"}`}
            >
              {meeting.type}
            </div>
            {meeting.name}
            ...
            <Button
              ...
              onClick={() => {
                ...
                const hasValidCoords =
                  meeting.coordinates &&
                  (meeting.coordinates.lat !== 0 || meeting.coordinates.lng !== 0);

                const mapsUrl =
                  hasValidCoords && meeting.coordinates
