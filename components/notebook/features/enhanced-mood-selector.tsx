...
export function EnhancedMoodSelector({ value, onChange, showKeyboardShortcuts = true }: EnhancedMoodSelectorProps) {
  ...
  return (
    <div className="overflow-visible px-8 -mx-8 py-4 -my-4">
      <div className="flex justify-between gap-1 mb-2 overflow-visible" role="group" aria-label="Mood selection">
        {moods.map((m) => {
          ...
          return (
            ...
            >
              ...
              {isSelected && (
                <div className={`absolute inset-0 ${m.bg} opacity-50 blur-xl rounded-lg -z-10 animate-pulse`} />
              )}

              <span className="text-3xl md:text-4xl filter drop-shadow-sm transition-transform duration-200" aria-hidden="true">
                {m.emoji}
              </span>
              <span className={`font-body text-xs ${m.color} mt-1 font-medium`}>
                {m.label}
              </span>
