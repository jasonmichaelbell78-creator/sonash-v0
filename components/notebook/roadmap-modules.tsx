...
export const notebookModules: NotebookModule[] = [
  {
    ...
    render: ({ nickname, onNavigate }) => (
      <TodayPage
        nickname={nickname || "Friend"}
        onNavigate={onNavigate ? (id) => onNavigate(id) : () => {}}
      />
    ),
  },
  {
  ...
  },
  {
    ...
    render: () => (
      <PlaceholderPage title="Support" description="Emergency contacts and crisis resources." />
    ),
  },
  {
    id: "work",
  ...
  },
  {
    ...
    render: () => (
      <PlaceholderPage title="More" description="Additional features and settings coming soon." />
    ),
  },
  {
    id: "history",
...
export const getModuleById = (id: NotebookModuleId) =>
  notebookModules.find((module) => module.id === id);

export const moduleIsEnabled = (module: NotebookModule) => {
  const enabled =
    module.status === "available" ||
    (module.featureFlag ? featureFlagEnabled(module.featureFlag) : false);
  return enabled;
};
