...
export default function TodayPage({ nickname, onNavigate }: TodayPageProps) {
  ...
  const { showCheckInReminder, showHaltSuggestion, showNoCravingsStreak, dismissPrompt } =
    useSmartPrompts({
      mood,
      cravings,
      _used: used,
  ...
  const _checkInSteps = useMemo(() => {
    const steps = [
      ...
      { id: "check", label: "Check", completed: cravings !== null && used !== null },
      {
        id: "halt",
        label: "HALT",
        completed: haltSubmitted || Object.values(haltCheck).some((v) => v),
      },
    ];
  ...
  const createJournalDailyLog = useCallback(
    async (data: {
      journalEntry: string;
      mood: string | null;
      cravings: boolean | null;
  ...
  const performSave = useCallback(async () => {
    ...
    try {
      ...
      if (
        !celebratedThisSessionRef.current &&
        dataToSave.mood &&
      ...
      <div className="h-full overflow-y-auto pr-2 pb-8 scrollbar-hide">
        ...
        <div className="flex flex-col md:flex-row gap-6 md:gap-8 items-start">
          ...
          <div className="w-full md:w-1/2 flex flex-col gap-6">
            {/* Check-in */}
            <div>
              <h2 className="font-heading text-xl text-amber-900/90 mb-3">
                ...
                {[
                  {
                    ...
                    label: "Hungry?",
                    icon: "🍽️",
                    tip: "When did you last eat? Grab a healthy snack.",
                  },
                  {
                    key: "angry" as const,
                    label: "Angry?",
                    icon: "😤",
                    tip: "What's bothering you. Call your sponsor.",
                  },
                  {
                    key: "lonely" as const,
                    label: "Lonely?",
                    icon: "🤝",
                    tip: "Reach out to someone. Attend a meeting.",
                  },
                  {
                    key: "tired" as const,
                    label: "Tired?",
                    icon: "😴",
                    tip: "How much sleep did you get? Take a break.",
                  },
                  ...
                  <div key={key} className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <label
                        htmlFor={`halt-${key}`}
                        className="font-handlee text-lg text-amber-900 flex items-center gap-2 cursor-pointer"
                      >
                        <span>{icon}</span>
                        <span>{label}</span>
                      </label>
            ...
            <div>
              <button
                ...
                className={`w-full py-6 px-4 rounded-xl font-heading text-2xl transition-all shadow-lg ${
                  hasCelebratedToday
            ...
            <div>
              ...
              <div className="bg-amber-50/50 border border-amber-100 rounded-lg p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="font-body text-amber-900/70">Check-ins this week</span>
                  <span className="data-display text-3xl text-amber-900">
                ...
                <div className="flex items-center justify-between pt-3 border-t border-amber-100">
                  ...
                  <span className="data-display text-3xl text-amber-900">
                    {weekStats.streak} {weekStats.streak === 1 ? "day" : "days"}
                  </span>
                </div>
              </div>
            </div>
