...
/**
 * State Manager Tests for Doc Ecosystem Audit
 *
...
function makeTempRoot() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "doc-audit-sm-test-"));
}
...
function makeSampleEntry(score) {
  return {
    ...
    categories: {
      internal_link_health: { score, rating: "good" },
      content_freshness: { score, rating: "average" },
    },
...
test("appendEntry returns true and entry is readable", () => {
  ...
  try {
    const sm = createStateManager(tmpRoot, () => true);
    const saved = sm.appendEntry(makeSampleEntry(79));
    assertEqual(saved, true, "appendEntry should return true");
    const entries = sm.readEntries();
    assertEqual(entries.length, 1, "Should have 1 entry");
    assertEqual(entries[0].healthScore.score, 79, "Score must match");
  } finally {
    cleanupTempRoot(tmpRoot);
  }
});
...
test("appendEntry returns false when guard rejects", () => {
  ...
  try {
    ...
    assertEqual(sm.appendEntry(makeSampleEntry(65)), false, "Must return false");
  } finally {
    cleanupTempRoot(tmpRoot);
  }
});
...
test("saveBaseline returns true and loadBaseline returns saved entry", () => {
  const tmpRoot = makeTempRoot();
  try {
    const sm = createStateManager(tmpRoot, () => true);
    const entry = makeSampleEntry(84);
    ...
    assertEqual(loaded.healthScore.score, 84, "Loaded score must match");
  } finally {
    cleanupTempRoot(tmpRoot);
  }
});
...
test("saveBaseline overwrites previous baseline", () => {
  ...
  try {
    ...
    assertEqual(loaded.healthScore.score, 88, "Should have the latest baseline");
  } finally {
    cleanupTempRoot(tmpRoot);
  }
});
...
test("getCompositeHistory returns scores in insertion order", () => {
  ...
  try {
    const sm = createStateManager(tmpRoot, () => true);
    for (const s of [65, 73, 81]) sm.appendEntry(makeSampleEntry(s));
...
test("getCategoryHistory extracts per-category scores", () => {
  ...
  try {
    ...
    const history = sm.getCategoryHistory("internal_link_health", 10);
...
test("getCompositeHistory respects window size", () => {
  ...
  try {
    ...
    const history = sm.getCompositeHistory(4);
    assertEqual(history.length, 4, "windowSize=4 should return 4 entries");
    assertEqual(history[3], 60, "Last should be most recent");
  } finally {
    cleanupTempRoot(tmpRoot);
  }
});
...
test("computeDelta computes correct scoreDelta", () => {
  ...
  try {
    const sm = createStateManager(tmpRoot, () => true);
    sm.appendEntry(makeSampleEntry(72));
    const delta = sm.computeDelta({
      healthScore: { score: 85, grade: "B" },
      categories: { internal_link_health: { score: 85, rating: "good" } },
    });
    assert(delta !== null, "Delta must be non-null");
    assertEqual(delta.scoreBefore, 72, "scoreBefore");
    assertEqual(delta.scoreAfter, 85, "scoreAfter");
    assertEqual(delta.scoreDelta, 13, "scoreDelta");
  } finally {
    cleanupTempRoot(tmpRoot);
  }
});
