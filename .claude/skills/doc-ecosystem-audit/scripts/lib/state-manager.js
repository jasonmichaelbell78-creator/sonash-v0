...
/**
 * State manager for Doc Ecosystem Audit.
 *
 * Handles reading/writing .claude/state/doc-ecosystem-audit-history.jsonl,
 * computing deltas between runs, and trend extraction.
 *
 * Forked from hook-ecosystem-audit — changed state file path.
 */
...
function createStateManager(rootDir, isSafeToWrite) {
  ...
  const STATE_FILE = path.join(STATE_DIR, "doc-ecosystem-audit-history.jsonl");
  ...
  function saveBaseline(entry) {
    const baselinePath = path.join(STATE_DIR, "doc-audit-baseline.json");
    try {
      if (!fs.existsSync(STATE_DIR)) {
        fs.mkdirSync(STATE_DIR, { recursive: true });
      }
