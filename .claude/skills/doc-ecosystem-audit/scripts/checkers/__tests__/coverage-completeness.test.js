...
function makeTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "dea-cc-"));
}
function removeTempDir(dir) {
  fs.rmSync(dir, { recursive: true, force: true });
}

const CHECKER_PATH = path.join(__dirname, "..", "coverage-completeness.js");
...
describe("coverage-completeness checker", () => {
  ...
  describe("happy path — complete coverage", () => {
    let tmpDir;
    before(() => {
      tmpDir = makeTempDir();
      fs.mkdirSync(path.join(tmpDir, "docs"), { recursive: true });
      // Docs covering key areas
      fs.writeFileSync(
        path.join(tmpDir, "docs", "getting-started.md"),
        "# Getting Started\n\nHow to get started.\n"
      );
      fs.writeFileSync(
        path.join(tmpDir, "docs", "api-reference.md"),
        "# API Reference\n\nAll endpoints.\n"
      );
      fs.writeFileSync(
        path.join(tmpDir, "docs", "troubleshooting.md"),
        "# Troubleshooting\n\nCommon issues.\n"
      );
    ...
    it("returns domain, findings array, scores object", () => {
      ...
      assert.equal(result.domain, "coverage_completeness");
  ...
  describe("empty project", () => {
    ...
    it("returns valid structure with empty project", () => {
      const { run } = require(CHECKER_PATH);
      const result = run({ rootDir: tmpDir });
      assert.ok(Array.isArray(result.findings));
      assert.ok(typeof result.scores === "object");
  ...
  describe("finding schema", () => {
    ...
    it("all findings have id, severity, message with DEA- prefix", () => {
      ...
      for (const f of result.findings) {
        assert.ok(/^DEA-\d+$/.test(f.id), `ID "${f.id}" must match DEA-NNN`);
