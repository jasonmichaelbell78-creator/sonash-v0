...
/**
 * State manager for Script Ecosystem Audit.
 *
 * Handles reading/writing .claude/state/script-ecosystem-audit-history.jsonl,
 * computing deltas between runs, and trend extraction.
 *
 * Forked from hook-ecosystem-audit — changed state file path and baseline name.
 */
...
function createStateManager(rootDir, isSafeToWrite) {
  ...
  const STATE_FILE = path.join(STATE_DIR, "script-ecosystem-audit-history.jsonl");
  ...
  function saveBaseline(entry) {
    const baselinePath = path.join(STATE_DIR, "script-audit-baseline.json");
    try {
      if (!fs.existsSync(STATE_DIR)) {
        fs.mkdirSync(STATE_DIR, { recursive: true });
      }
