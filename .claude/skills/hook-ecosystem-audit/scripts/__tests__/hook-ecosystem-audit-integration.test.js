...
/**
 * Integration Tests for Hook Ecosystem Audit
 *
 * Validates the v2 JSON output schema by running the full audit via
 * direct module composition (no subprocess spawn). Complements the
 * existing checker-regression.test.js which is domain/FP focused.
 *
 * Usage:
 *   node hook-ecosystem-audit-integration.test.js
 *
...
let compositeScore, impactScore, computeTrend;
let CATEGORY_WEIGHTS;
let checkers;

try {
  ({ compositeScore, impactScore, computeTrend } = require(
    path.join(SCRIPTS_DIR, "lib", "scoring")
  ));
  ({ CATEGORY_WEIGHTS } = require(path.join(SCRIPTS_DIR, "lib", "benchmarks")));
  checkers = [
    require(path.join(SCRIPTS_DIR, "checkers", "code-quality-security")),
    require(path.join(SCRIPTS_DIR, "checkers", "functional-correctness")),
    require(path.join(SCRIPTS_DIR, "checkers", "config-health")),
    require(path.join(SCRIPTS_DIR, "checkers", "precommit-pipeline")),
    require(path.join(SCRIPTS_DIR, "checkers", "state-integration")),
  ];
} catch (err) {
  console.error(
...
// Also try loading cicd-pipeline (optional checker)
let cicdPipeline;
try {
...
function buildAuditResult() {
  const allFindings = [];
  const allScores = {};
  const allCheckers = cicdPipeline ? [...checkers, cicdPipeline] : checkers;
  ...
  for (const checker of allCheckers) {
    try {
      ...
      allFindings.push({
        id: `HEA-FAIL-${checker.DOMAIN || "unknown"}`,
...
test("result.findings is sorted array", () => {
  ...
  for (let i = 1; i < findings.length; i++) {
    ...
    assert(prev >= curr, `findings[${i - 1}](${prev}) must be >= findings[${i}](${curr})`);
  }
});

test("result.summary has non-negative errors/warnings/info", () => {
  const { summary } = auditResult;
  assert(summary.errors >= 0 && summary.warnings >= 0 && summary.info >= 0, "All counts >= 0");
});

test("summary total equals findings array length", () => {
...
test("result.patchableCount is non-negative number", () => {
  ...
  assert(auditResult.patchableCount >= 0, "patchableCount must be >= 0");
});
...
test("all CATEGORY_WEIGHTS categories appear in output", () => {
...
test("all categories have a rating (good/average/poor)", () => {
  for (const [cat, scoreObj] of Object.entries(auditResult.categories)) {
    assert(
      ["good", "average", "poor"].includes(scoreObj.rating),
      `${cat}.rating must be good/average/poor, got: ${scoreObj.rating}`
    );
  }
});
...
test("all findings have valid severity (error/warning/info)", () => {
  const valid = ["error", "warning", "info"];
  for (const f of auditResult.findings) {
    assert(valid.includes(f.severity), `Finding ${f.id} has invalid severity: ${f.severity}`);
  }
});
...
test("orchestrator documents --check, --summary, --batch, --save-baseline", () => {
  const orch = path.join(SCRIPTS_DIR, "run-hook-ecosystem-audit.js");
...
test("orchestrator uses compositeScore and CATEGORY_WEIGHTS", () => {
  const orch = path.join(SCRIPTS_DIR, "run-hook-ecosystem-audit.js");
  const content = fs.readFileSync(orch, "utf8");
  assert(content.includes("compositeScore"), "Must call compositeScore");
  assert(content.includes("CATEGORY_WEIGHTS"), "Must use CATEGORY_WEIGHTS");
});

test("orchestrator loads core checkers", () => {
  const orch = path.join(SCRIPTS_DIR, "run-hook-ecosystem-audit.js");
  const content = fs.readFileSync(orch, "utf8");
  const coreCheckers = [
  ...
  for (const c of coreCheckers) {
    assert(content.includes(c), `Orchestrator must load checker: ${c}`);
  }
});

// ============================================================================
// TEST GROUP 5: computeTrend integration
// ============================================================================

console.log("\n--- Test Group 5: computeTrend Integration ---");

test("computeTrend returns improving for upward sequence", () => {
  const trend = computeTrend([60, 65, 70, 75, 80]);
  assert(trend !== null, "Must return trend");
  assertEqual(trend.direction, "improving", "Should be improving");
});

test("computeTrend returns null for single historical + current", () => {
  // Simulates the orchestrator pattern: history (n=1) + current
  const historyPlusCurrentOf2 = [75, 80];
  const trend = computeTrend(historyPlusCurrentOf2);
  // 2 values: (80-75)/75 = 6.67% > 5% threshold → improving
  assert(trend !== null, "2 values can compute trend");
  assertEqual(trend.direction, "improving", "Should be improving");
});
