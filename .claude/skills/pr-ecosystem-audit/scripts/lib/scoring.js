...
/**
 * Scoring utilities for PR Ecosystem Audit
 *
...
function scoreMetric(value, benchmark, direction = "lower-is-better") {
  ...
  if (direction === "lower-is-better") {
    if (value <= benchmark.good) {
      ...
      rating = "average";
      // Linear interpolation between good and average
 ...
 * @param {Object<string, { score: number }>} categoryScores - Map of category -> { score }
 * @param {Object<string, number>} weights - Map of category -> weight (should sum to 1.0)
...
function impactScore(finding) {
  ...
  const freqRaw = Number(finding.frequency ?? 1);
  const blastRaw = Number(finding.blastRadius ?? 1);

  const freq = Math.max(0, Math.min(1.0, Number.isFinite(freqRaw) ? freqRaw / 10 : 0.1));
  const blast = Math.max(0, Math.min(1.0, Number.isFinite(blastRaw) ? blastRaw / 5 : 0.2));
