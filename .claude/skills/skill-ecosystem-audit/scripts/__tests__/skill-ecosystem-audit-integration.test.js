...
try {
  ...
  checkers = [
    require(path.join(SCRIPTS_DIR, "checkers", "structural-compliance")),
    require(path.join(SCRIPTS_DIR, "checkers", "cross-reference-integrity")),
    require(path.join(SCRIPTS_DIR, "checkers", "coverage-consistency")),
    require(path.join(SCRIPTS_DIR, "checkers", "staleness-drift")),
    require(path.join(SCRIPTS_DIR, "checkers", "agent-orchestration")),
  ];
} catch (err) {
  console.error(
...
function buildAuditResult() {
  ...
  for (const checker of checkers) {
    try {
      ...
      allFindings.push({
        id: `SKEA-FAIL-${domain}-${runtimeFailIdx}`,
...
test("orchestrator documents --check, --summary, --batch, --save-baseline", () => {
  const orch = path.join(SCRIPTS_DIR, "run-skill-ecosystem-audit.js");
