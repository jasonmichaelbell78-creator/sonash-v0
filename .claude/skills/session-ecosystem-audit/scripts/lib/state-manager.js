...
/**
 * State manager for Session Ecosystem Audit.
 *
 * Handles reading/writing .claude/state/session-ecosystem-audit-history.jsonl,
 * computing deltas between runs, and trend extraction.
 *
 * Forked from tdms-ecosystem-audit — changed state file path.
 */
...
function createStateManager(rootDir, isSafeToWrite) {
  ...
  const STATE_FILE = path.join(STATE_DIR, "session-ecosystem-audit-history.jsonl");
