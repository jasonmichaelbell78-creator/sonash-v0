...
For findings claiming "file X doesn't exist" or "doc Y is missing":

1. **MUST include actual command output** in evidence (e.g.,
   `ls -la <file> 2>&1`)
2. **Check document references** - If a doc references another file, verify it
   exists
...
- **Existence claims** ("file missing", "doc absent"): Max MEDIUM unless
  tool-validated
- **Negative assertions** ("no X found"): Max MEDIUM - absence of evidence ≠
  evidence of absence
- **Link validation**: Require actual fetch/read, not assumption
...
```json
{
  "verification_steps": {
    "first_pass": {
      ...
      "evidence_collected": ["broken link text", "grep output"]
    },
    "second_pass": {
      "method": "contextual_review|exploitation_test|manual_verification",
      "confirmed": true,
      "notes": "Re-read in context, verified documentation issue"
    },
    "tool_confirmation": {
      "tool": "eslint|sonarcloud|npm_audit|patterns_check|typescript|NONE",
      "reference": "docs:check output or NONE with justification"
    }
  }
}
```
...
4. **Pre-Backlog Verification (Session #99 Enhancement):** Before adding
   ...
   - **Existence findings**: Re-run `ls -la <file>` to confirm file is still
     missing
   - **Link findings**: Re-test the link to confirm it's still broken
   - **Reference findings**: Verify referenced docs still exist If a finding
...
6. **Update AUDIT_TRACKER.md** - Add entry to "Documentation Audits" table:
   - Date: Today's date
...
8. Ask: "Would you like me to fix any of these documentation issues now?"
