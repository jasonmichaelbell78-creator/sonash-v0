...
const securityKeywords = /(^|[^a-z0-9])(auth|token|credential|secret|password|apikey|api-key|jwt|oauth|session|encrypt|crypto)([^a-z0-9]|$)/;
...
const hasSensitiveName = /(secret|credential|auth|key|token|password)/.test(filename) || filename.startsWith('.env');
