...
description:
  hook-checks.json schema (D14) must be registered in SWS CANON plan — defines
  hook check contract for all ecosystems
type: project
status: active
---

The hook-checks.json schema (D14) is a CANON-level artifact per T5 (contract
over implementation). Defines what every hook check must declare: id, command,
blocking behavior, exit codes, conditions, actions. All ecosystems register
checks against this schema.
