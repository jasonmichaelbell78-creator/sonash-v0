...
- `/gsd:plan-phase --gaps` orchestrator (gap closure planning from verification
  failures)
- `/gsd:plan-phase` orchestrator in revision mode (updating plans based on
  checker feedback)
...
Each task should take Claude **15-60 minutes** to execute. This calibrates
granularity:
...
**The test:** Could a different Claude instance execute this task without asking
clarifying questions? If not, add specificity.
...
**Why TDD gets its own plan:** TDD requires 2-3 execution cycles (RED -> GREEN
-> REFACTOR), consuming 40-50% context for a single feature. Embedding in
multi-task plans degrades quality.
...
- New SDK: `stripe`, `@sendgrid/mail`, `twilio`, `openai`,
...
(account creation, secret retrieval, dashboard config).

**Important:** User setup info goes in frontmatter ONLY. Do NOT surface it in
your planning output or show setup tables to users. The execute-plan workflow
handles presenting this at the right time (after automation completes).
...
- `needs`: What must exist before this task runs (files, types, prior task
  outputs)
...
If file appears in multiple plans: Later plan depends on earlier (by plan
number).
...
**Key principle:** Derive plans from actual work. Depth determines how
...
Don't pad small work to hit a number. Don't compress complex work to look
efficient.
...
**Wave is pre-computed:** Wave numbers are assigned during planning.
...
Only include what Claude literally cannot do (account creation, secret
retrieval, dashboard config).
...
**Step 1: State the Goal** Take the phase goal from ROADMAP.md. This is the
...
**Step 2: Derive Observable Truths** Ask: "What must be TRUE for this goal to be
achieved?"
...
**Step 3: Derive Required Artifacts** For each truth, ask: "What must EXIST for
this to be true?"
...
**Step 4: Derive Required Wiring** For each artifact, ask: "What must be
CONNECTED for this artifact to function?"
...
When Claude tries CLI/API and gets auth error, this is NOT a failure - it's a
gate.

Pattern: Claude tries automation -> auth error -> creates checkpoint -> user
authenticates -> Claude retries -> continues

Authentication gates are created dynamically when Claude encounters auth errors
during automation. They're NOT pre-planned.
...
Each phase involves file reads, test runs, output analysis. The back-and-forth
is heavier than linear execution.
...
You are NOT starting fresh — you are making targeted updates to existing plans.
...
If multiple phases available, ask which one to plan. If obvious (first
incomplete phase), proceed.
...
**If CONTEXT.md exists:** Honor user's vision, prioritize their essential
features, respect stated boundaries. These are locked decisions - do not
revisit.

**If RESEARCH.md exists:** Use standard_stack, architecture_patterns,
dont_hand_roll, common_pitfalls. Research has already identified the right
tools. </step>
...
Write to `.planning/phases/XX-name/{phase}-{NN}-PLAN.md` (e.g., `01-02-PLAN.md`
for Phase 1, Plan 2)

Include frontmatter (phase, plan, type, wave, depends_on, files_modified,
autonomous, must_haves). </step>
