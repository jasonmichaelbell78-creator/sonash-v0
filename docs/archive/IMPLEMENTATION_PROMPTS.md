...
**Document Version**: 2.1 **Created**: 2025-12-30 **Last Updated**: 2026-01-05
**Status**: ARCHIVED
...
**Reason**: The EIGHT_PHASE_REFACTOR_PLAN.md that this document supported has
been completed and archived. These implementation prompts are preserved for
historical reference but are no longer actively used.
...
- [MULTI_AI_REFACTOR_PLAN_TEMPLATE.md](../templates/MULTI_AI_REFACTOR_PLAN_TEMPLATE.md) -
  Refactoring process
- [MULTI_AI_REVIEW_COORDINATOR.md](../MULTI_AI_REVIEW_COORDINATOR.md) - Review
  coordination
...
This document contains reusable prompts for implementing and reviewing each
phase of the refactoring plan.

**Related Documents**:

- [EIGHT_PHASE_REFACTOR_PLAN.md](./completed-plans/EIGHT_PHASE_REFACTOR_PLAN.md) -
  Master tracking document (archived)
- **[PR_WORKFLOW_CHECKLIST.md](../PR_WORKFLOW_CHECKLIST.md) - START HERE!
  Complete workflow with checkboxes**
...
1. [Master PR Implementer Prompt](#master-pr-implementer-prompt) - Step 1:
   Implementation
...
**Usage**: Use this prompt for implementing any of the 8 phases (PR1-PR8).
Combine with the specific PR payload from EIGHT_PHASE_REFACTOR_PLAN.md.

---

## ROLE

You are the Implementation Engineer for a single PR in a Next.js (App Router) +
React + TypeScript + Firebase repo. You are executing one PR from a deduped
refactor plan focused on cross-cutting duplication/inconsistency.
...
2. **CANON_FINDINGS_JSONL** (one JSON object per line) - Canonical findings to
   satisfy in this PR
...
- **Do NOT re-audit the whole repo**. Implement ONLY what's required by
  PR_OBJECT + CANON_FINDINGS_JSONL.
...
- **Evidence discipline**: Every claim about a change must cite file path +
  symbol name you touched.
- **No rewrites**. Prefer extraction + mechanical migration.
- **No secrets**. Never print env values or keys.
- **Decision Discipline**: When you encounter an architectural blocker or
  ambiguous requirement:
  1. **Do NOT guess or pick the "easiest" option** - document all viable
     approaches
  2. **Use structured decision framework**: List 2-4 options with
     pros/cons/trade-offs
  3. **Document decision criteria**: Security, user impact, development
     velocity, risk management
  4. **Make a recommendation** with clear rationale, but mark as "NEEDS
     APPROVAL" if it affects security posture or user experience
  5. **Update tracking document** with the decision, decision owner, timeline,
     and fallback plan
  6. **Example**: See CANON-0002 App Check decision in
     EIGHT_PHASE_REFACTOR_PLAN.md (Options A/B/C analysis → Option D hybrid)
...
2. Prefer shared helpers/utilities when `duplication_cluster` indicates a
   cluster
3. After each coherent chunk, run targeted checks (lint/typecheck/tests
   relevant)
...
**Usage**: Use this after completing any phase to catch regressions, scope
creep, and hidden duplication.

---

## ROLE

You are a senior reviewer. Your job is to catch regressions, scope creep, and
hidden duplication.
...
1. **Does the PR actually satisfy the PR_OBJECT goal and the included CANON
   IDs?**
...
**Purpose**: Steps to take after completing each phase, before starting the
next.
...
Document the new canonical surface in EIGHT_PHASE_REFACTOR_PLAN.md using this
format:
...
- Example: Direct Firestore writes: `setDoc(doc(db, 'users/...'))` - bypasses
  validation and rate limiting

**Verification Commands**:

- `grep -r "pattern-to-avoid" .` should return zero results in relevant
  directories
```
...
- Calling Cloud Functions directly with
  `httpsCallable(functions, 'saveJournalEntry')`
...
Prevents the next AI PR from reintroducing the old pattern. Makes the "canonical
surface" concrete and greppable.
...
After PRs that change a "surface" (Firestore paths, callables, auth), do 2–3
quick searches to ensure the old path/pattern is gone.
...
This is the fastest way to stop "cross-cutting inconsistency" from creeping
back.
...
Do NOT ask the aggregator model to re-scan the whole repo between PRs. Instead,
feed it the PR diff summary + changed files + any new helper APIs.
Re-aggregating wastes tokens and causes dedupe noise.
...
1. **Verify Firestore Rules + App Check behavior** in your expected envs
   (dev/stage/prod)
2. **If using emulators**: Run emulator tests or a quick manual emulator session

**Because**: These PRs change trust boundaries and are the easiest to "think"
you fixed while leaving a bypass.
