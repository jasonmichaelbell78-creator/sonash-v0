...
| ------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 10.0    | 2026-01-20 | Review #187: PR #287 Qodo Compliance + Suggestions Round 2 (22 suggestions). **CRITICAL (2)**: Sync error handling in use-tab-refresh.ts (.then() pattern), security scanner missing matches (always add 'g' flag). **MAJOR (7)**: Security fixes - captureToSentry false for IP hash, hash attemptedUserId, sanitize error messages in logs, symlink detection with lstatSync, searchUserByEmail error logging. **MEDIUM (8)**: Robustness - non-string path guard, multi-column separator regex, try/catch processFileForMapping, outputCrossRefResults error handling, parseInt radix, Windows path normalization, Array.isArray guards, cap search text. **MINOR (5)**: Reject positional args, use eventData in output, strict boolean check, prevent duplicate bucket indices. **KEY PATTERNS**: (1) Promise.resolve().then() catches both sync and async errors; (2) Security scanners must use global regex flag; (3) Hash user identifiers before logging; (4) Use lstatSync to detect symlinks; (5) Cap input sizes for regex operations. |
...
**Status:** ✅ Current **Next consolidation due:** After Review #187
...
#### Review #187: PR #287 Qodo Compliance + Suggestions Round 2 (2026-01-20)

**Source:** Qodo PR Code Suggestions + Compliance Guide **PR/Branch:**
claude/new-session-Qy21d (PR #287) **Suggestions:** 22 total (Critical: 2,
Major: 7, Medium: 8, Minor: 5)

**Issues Fixed:**

| #   | Issue                                  | Severity | File                            | Fix                                        |
| --- | -------------------------------------- | -------- | ------------------------------- | ------------------------------------------ |
| 1   | Sync error not caught                  | Critical | use-tab-refresh.ts:68           | Use Promise.resolve().then() pattern       |
| 2   | Scanner misses multiple matches        | Critical | security-check.js:154           | Always add 'g' flag to find all matches    |
| 3   | IP hash to Sentry is PII risk          | Major    | security-wrapper.ts:132         | Set captureToSentry: false                 |
| 4   | attemptedUserId logged in cleartext    | Major    | security-wrapper.ts:266         | Hash with hashUserId()                     |
| 5   | Error string may contain PII           | Major    | admin.ts:2521                   | Add sanitizeErrorMessage() helper          |
| 6   | searchUserByEmail swallows errors      | Major    | admin.ts:139                    | Log unexpected errors (not user-not-found) |
| 7   | Symlink path traversal risk            | Major    | validate-canon-schema.js:385    | Use lstatSync + skip symlinks              |
| 8   | Non-string d.path causes crash         | Medium   | phase-complete-check.js:181     | Add typeof guard clause                    |
| 9   | Separator regex misses multi-column    | Medium   | update-readme-status.js:190     | Fix regex for multi-column tables          |
| 10  | processFileForMapping throws unhandled | Medium   | normalize-canon-ids.js:307      | Add try/catch with clean exit              |
| 11  | err.message crash on non-Error         | Medium   | normalize-canon-ids.js:215      | Use instanceof check                       |
| 12  | title.substring crash on non-string    | Medium   | normalize-canon-ids.js:235      | Add typeof check for title                 |
| 13  | outputCrossRefResults crashes on fail  | Medium   | archive-doc.js:527              | Check success and Array.isArray            |
| 14  | parseInt without radix                 | Medium   | check-docs-light.js:90          | Use Number.parseInt(tier, 10)              |
| 15  | Windows paths break folder matching    | Medium   | check-docs-light.js:127         | Normalize with replaceAll("\\", "/")       |
| 16  | Malformed API crashes parseIssues      | Medium   | check-review-needed.js:267      | Add Array.isArray guards                   |
| 17  | Large input ReDoS risk                 | Medium   | validate-audit.js:193           | Cap evidence items and total size          |
| 18  | Positional args silently ignored       | Minor    | add-false-positive.js:282       | Throw on unexpected args                   |
| 19  | Output shows raw args not eventData    | Minor    | log-session-activity.js:454     | Use eventData for accurate output          |
| 20  | Truthy check on isSoftDeleted          | Minor    | use-journal.ts:174              | Use strict === true check                  |
| 21  | Duplicate bucket indices               | Minor    | aggregate-audit-findings.js:707 | Skip adjacent duplicates                   |

**Patterns Identified:**

1. **Promise.resolve(fn()) doesn't catch sync throws**: Use `.then(() => fn())`
   to catch both synchronous and asynchronous errors
2. **Non-global regex in scanner only finds first match**: Security scanners
   should always use global flag to find all occurrences
3. **Hashed identifiers still considered derived PII**: Even hashed values can
   be considered PII under some regulations - keep out of third-party services
4. **lstatSync reveals symlinks without following them**: Use instead of
   statSync to detect and skip symlinks for security
5. **Cap input sizes for regex operations**: Large inputs can cause performance
   issues even with safe patterns

**Key Learnings:**

- Promise error handling: Promise.resolve().then() is safer than
  Promise.resolve(fn())
- Security scanning: Non-global regex patterns miss multiple occurrences in a
  file
- PII compliance: Hashed values are still derived data - conservative approach
  is safer
- Cross-platform: Always normalize Windows backslashes for path matching
- Defensive coding: Guard against non-string/non-array values from untrusted
  input

**Resolution:**

- Fixed: 21 items (2 Critical, 7 Major, 8 Medium, 4 Minor)
- Files modified: 18 files across scripts, functions, hooks, and lib
