...
**File:** `.claude/commands/session-begin.md` **Lines:** 89 **Created:** Session
#20
...
**File:** `.claude/commands/session-end.md` **Lines:** 150 **Created:** Session
#20
...
**File:** `.claude/commands/pr-review.md` **Lines:** 300 **Created:** Session
#22
...
5. **Auto-Invoke `/pr-review`** - Automatically proceeds with full PR review
   protocol (categorization, TodoWrite, agents, learning capture, verification)
...
**Priority:** CRITICAL **Estimated Implementation:** 2-3 hours **Gap Closed:**
Pattern consolidation is manual 7-step process, often skipped
...
---
description:
  Consolidate review patterns into claude.md (when 10+ reviews accumulated)
---
...
- If < 10: "Consolidation not due ($counter/10 reviews). Use --force to
  override."
...
**Priority:** CRITICAL **Estimated Implementation:** 2-3 hours **Gap Closed:**
Phase completion audits are manual checklists, often incomplete

#### Description

Automates the Step Completion Audit required before marking any
INTEGRATED_IMPROVEMENT_PLAN.md step as COMPLETE.
...
Step audits require manual verification of each deliverable, running validation
scripts, and generating audit tables. This is error-prone and often skipped
under time pressure.
...
**Audit performed by:** Claude **Audit date:** YYYY-MM-DD
...
**Priority:** CRITICAL **Estimated Implementation:** 2 hours **Gap Closed:** No
automated verification of 50+ security patterns in claude.md
...
50+ security patterns documented in claude.md Section 4 + CODE_PATTERNS.md, but
no automated verification. ESLint security baseline (181 warnings) can drift
without detection.
...
**Priority:** CRITICAL **Estimated Implementation:** 1.5 hours **Gap Closed:**
Ambiguous "when should we run multi-AI review?" decisions
...
Multiple trigger thresholds exist (50 commits, 10 reviews, 5 security files, 30
days) but no unified view. Decision to run review is subjective.
...
**Estimated Duration:** 4-6 hours **Suggested Models:** Claude Opus 4.5,
...
**Priority:** CRITICAL **Estimated Implementation:** 2-3 hours **Gap Closed:**
No workflow for executing CANON findings from Step 4
...
**File:** lib/firebase.ts:45 **Suggested Fix:**
...
**Commit:** "fix(security): CANON-0012, CANON-0015 - input validation in
firebase.ts"
...
**Priority:** HIGH **Estimated Implementation:** 1.5 hours **Gap Closed:**
Review #53 - CI broke when script referenced archived file
...
When DOCUMENTATION_STANDARDIZATION_PLAN.md was archived,
validate-phase-completion.js still referenced it, causing CI failure. Manual
grep for references is error-prone.
...
**Priority:** HIGH **Estimated Implementation:** 1.5 hours **Gap Closed:**
Session learnings not captured consistently
...
**Source:** Session debugging **Suggestions:** 2 (Critical: 1, Major: 1)

**Patterns Identified:**

1. [Archive Reference Breaking CI]: Scripts and workflows that reference docs
   must be updated when those docs are archived
...
**Priority:** HIGH **Estimated Implementation:** 1 hour **Gap Closed:** ESLint
181 warning baseline can drift undetected
...
**Priority:** HIGH **Estimated Implementation:** 1 hour **Gap Closed:** Broken
links discovered late after doc moves
...
**Priority:** HIGH **Estimated Implementation:** 1 hour **Gap Closed:** Multiple
trigger thresholds need unified view
...
**Priority:** MEDIUM **Estimated Implementation:** 1.5 hours
...
**Priority:** MEDIUM **Estimated Implementation:** 1 hour
...
**Priority:** MEDIUM **Estimated Implementation:** 0.5 hours
...
**Priority:** HIGH (for Step 4) **Estimated Implementation:** 2 hours
...
> **Note:** This table reflects the current pre-implementation state. Commands
> listed in the Priority Implementation Order (e.g., `/consolidate-patterns`,
> `/security-check`, `/remediate`) are designed to address the CRITICAL and
> other gaps shown below.
...
1. **Phase 1 (Week 1):** `/consolidate-patterns`, `/audit-step`,
   `/security-check`
...
- [INTEGRATED_IMPROVEMENT_PLAN.md](./INTEGRATED_IMPROVEMENT_PLAN.md) - Current
  improvement plan
