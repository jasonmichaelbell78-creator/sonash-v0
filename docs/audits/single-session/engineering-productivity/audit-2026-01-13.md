...
**Date:** 2026-01-13
**Auditor:** Staff+ Engineering Productivity Specialist
...
- Use browser DevTools → Network → filter by `/functions/` for Cloud Function calls
...
- **Verification:** `npm run dev:offline`, verify `localhost:3000` and `localhost:4000` work

#### EFF-002: Create `docs/DEV_WORKFLOW.md`

- **Evidence:** Info scattered across `DEVELOPMENT.md` (759 lines), `README.md`, `AI_WORKFLOW.md`
...
- **Verification:** `npm run doctor` passes on working setup, fails clearly on broken setup
...
- **Verification:** Make request, find same correlation ID in frontend and backend logs
...
- **Evidence:** `tests/firestore-service.test.ts:53-67` - tests skipped for Cloud Functions
...
- **Scripts in npm, not direct node calls** - `npm run seed` not `npx tsx scripts/seed.ts`
...
1. **Write when offline** → Cloud Function call fails → Error shown to user → Data lost
2. **Go offline mid-write** → Retry 3x with backoff → Eventually fails → Data lost
3. **Reconnect after offline** → No sync → Previous writes lost
4. **Conflict (edited on two devices)** → No resolution → Last write wins (data loss)
...
**Recommendation:** Add `leaflet.markercluster` to dependencies or use CSS import from CDN.
