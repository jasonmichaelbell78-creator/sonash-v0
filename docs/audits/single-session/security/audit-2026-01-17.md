...
**Impact:** Clickjacking, MIME-sniffing attacks, protocol downgrade attacks, XSS risk increased.

**Recommendation:** Add comprehensive security headers to firebase.json. Start with permissive CSP and tighten iteratively.

---

### SEC-002: App Check disabled on Cloud Functions (S1) - CARRY-FORWARD

**File:** functions/src/index.ts:84
**OWASP:** A01:2021 Broken Access Control
**Status:** UNRESOLVED from audit-2026-01-13 (SEC-001)

Firebase App Check is disabled (`requireAppCheck: false`) on all Cloud Functions with comment "TEMPORARILY DISABLED - waiting for throttle to clear".

**Impact:** Attackers can call Cloud Functions directly without proving they're using the legitimate app.

**Recommendation:** Check if throttle period has cleared and re-enable App Check.
...
- Sensitive collections (journal, daily_logs, inventoryEntries) block direct writes
...
1. **Add security headers to firebase.json** (SEC-001) - Unresolved since 2026-01-13
2. **Check App Check throttle status** (SEC-002) - Re-enable if throttle cleared

### Short-term (This Week)

3. **Remove hardcoded reCAPTCHA fallback** (SEC-003)
4. **Review ReDoS patterns in production code** (SEC-006) - Especially admin.ts and security-logger.ts
