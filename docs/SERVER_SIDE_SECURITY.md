...
2. **Client-Side Validation Only**
   - Current: All validation in `lib/security/firestore-validation.ts` runs in browser
...
**Result:** Bots without valid App Check tokens are rejected by Firebase, not your client code.
  ...
  .https.onRequest(async (req, res) => {
    ...
    try {
      ...
      if (decodedToken.uid !== userId) {
        res.status(403).json({ error: "Forbidden: Cannot write another user's data" })
        return
      }
      ...
      const docRef = db.collection("users").doc(userId).collection("daily_logs").doc(today)
...
**Result:** Rate limiting enforced on server. Bypassing client code does nothing.

---

## Server-Side Authorization

**Purpose:** Complex authorization logic that can't be expressed in Firestore rules
...
export const shareJournalEntry = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    const entryRef = db.collection("users").doc(ownerId).collection("daily_logs").doc(entryId)
...
const maliciousWrite = await setDoc(doc(db, 'users', 'VICTIM_USER_ID', 'daily_logs', 'today'), {
  hacked: true
})
...
- **Firestore Security:** https://firebase.google.com/docs/firestore/security/rules-conditions
