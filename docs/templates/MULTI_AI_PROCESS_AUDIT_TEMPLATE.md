...
Category 3b: Script Trigger Coverage (NEW - Session #48) CHECKS: [ ] All scripts
have npm run commands (discoverability) [ ] Critical scripts have automatic
triggers (session-start, pre-commit, CI) [ ] Validation scripts run when
relevant files change [ ] Scripts that should run together are chained properly
[ ] Manual-only scripts are documented as intentionally manual [ ] No orphan
scripts (created but never integrated)
...
Mark each check: ISSUE | OK | N/A List scripts without appropriate automatic
triggers.
