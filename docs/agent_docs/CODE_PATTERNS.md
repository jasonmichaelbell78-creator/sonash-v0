...
| 🔴       | Cross-device rename safety    | Never use bare `renameSync` — always wrap in try/catch with `copyFileSync`+`unlinkSync` fallback. Agent fixes MUST use this pattern. | `renameSync` fails cross-device; bare rename after `rmSync` creates data-loss window (PR #427 R3 — agent regression) |
| 🟡       | Worktree .git compatibility   | Never use `$REPO/.git/` path — use `$(git rev-parse --git-dir)/` instead. `.git` is a file (not directory) in worktrees.             | Pre-commit hook broke in worktree context (PR #428 R1)                                                               |
...
### Pattern Checker Rename No Fallback Requires All 4 Elements

🟡 **Rule:** Pattern Checker Rename No Fallback Requires All 4 Elements —
recurring pattern from 3 reviews (#445, #447, #453)

**Source:** Consolidation #8 (Reviews #442-#453)
