...
```
<module>.test.ts                    # Unit test (TypeScript)
<module>.test.js                    # Unit test (JavaScript)
<module>.property.test.js           # Property-based test
<module>.integration.test.js        # Integration test
<module>.e2e.test.ts                # End-to-end test
```
...
3. **Mid-session alerts**: `scripts/health/mid-session-alerts.js` can flag test
   failures during active development sessions.
...
| `audit_checker`  | Ecosystem audit domain checker scripts                   | `.claude/skills/pr-ecosystem-audit/scripts/checkers/pr-quality.js` |
| `test_protocol`  | `.protocol.json` UI test protocol files                  | `tests/ui/daily-log.protocol.json`                                 |
