...
This document outlines security measures, privacy protections, and data handling
practices for the SoNash recovery app.
...
- [SERVER_SIDE_SECURITY.md](./SERVER_SIDE_SECURITY.md) - Cloud Functions
  security patterns
...
- **App Check**: reCAPTCHA Enterprise configured (⚠️ temporarily disabled in
  Cloud Functions due to Firebase 403 throttle - see
  EIGHT_PHASE_REFACTOR_PLAN.md CANON-0002)

### Layer 5: Monitoring & Audit

- **Sentry**: Error monitoring for client and Cloud Functions
- **Audit logging**: Security events logged to GCP Cloud Logging
- **Event types**: AUTH_FAILURE, RATE_LIMIT_EXCEEDED, APP_CHECK_FAILURE,
  VALIDATION_FAILURE, AUTHORIZATION_FAILURE
...
Device lost | Data orphaned forever | Account linking (planned) | | Browser
cleared | Data orphaned | Local backup warning | | No password | Can't recover
on new device | Email/OAuth linking (planned) |
...
**Recommendation:** Implement account linking before storing significant user
data.
...
Regular key rotation is a critical security practice that limits the impact of
compromised credentials. This section defines the rotation schedule and
procedures for all application keys and secrets.
...
\*Firebase Web API Keys are designed to be public and don't require rotation.
They're protected by Firestore Rules and App Check.

### Service Account Key Rotation

**CRITICAL:** Service account keys (`FIREBASE_ADMIN_PRIVATE_KEY`,
`FIREBASE_ADMIN_CLIENT_EMAIL`) have full access to Firebase resources and MUST
...
6. **Document Rotation**
   - Record rotation date in `docs/security/KEY_ROTATION_LOG.md` (create if
     needed)
...
If a service account key is compromised (committed to git, leaked in logs,
etc.):
...
6. **Update** [FIREBASE_CHANGE_POLICY.md](./FIREBASE_CHANGE_POLICY.md) if key
   exposure vector needs mitigation

**NEVER:**

- Commit service account keys to git (check with
  `git log -p | grep "private_key"`)
...
**Firebase Web API Keys are designed to be public** - they're safe to include in
client-side code. They're protected by:
...
1. **Create New reCAPTCHA Keys**
   - Go to
     [Google Cloud Console > reCAPTCHA Enterprise](https://console.cloud.google.com/security/recaptcha)
...
- ✅ Store keys in environment variables (`.env.local`, hosting provider env
  config)
- ✅ Use secret managers (Firebase Secret Manager, Google Secret Manager, Vercel
  Secrets)
...
**Note:** Cloud Functions secrets are defined using `defineSecret()` and
accessed via `.value()` at runtime. Never log secret values.
...
- Firebase Support:
  [Firebase Console](https://console.firebase.google.com/support)
...
4. ⚠️ Configure billing alerts (see
   [BILLING_ALERTS_SETUP.md](./archive/2025-dec-reports/BILLING_ALERTS_SETUP.md))
...
- [FIREBASE_CHANGE_POLICY.md](./FIREBASE_CHANGE_POLICY.md) - Firebase security
  review requirements
- [BILLING_ALERTS_SETUP.md](./archive/2025-dec-reports/BILLING_ALERTS_SETUP.md) -
